			...
			try
			{
				...
				while (true)
				{
					...
					if (!inventoryAction.IsUsable(Owner, GO, Distant, out var Telekinetic))
					{
						if (Telekinetic)
						{
							Popup.Show(GO.Does("are", int.MaxValue, null, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, WithIndefiniteArticle: false, null, IndicateHidden: false, Pronoun: false, SecondPerson: true, null) + " out of your telekinetic range.");
						}
						else
						{
