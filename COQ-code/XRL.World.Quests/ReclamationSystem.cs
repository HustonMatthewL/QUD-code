	...
	public class ReclamationSystem : IQuestSystem
	{
		...
		public override bool HandleEvent(EnteringZoneEvent E)
		{
			if (Active && Stage < 2 && !E.Forced && !E.System)
			{
				...
				if (!Perimeter.Contains(zone.ZoneID) && Popup.ShowYesNo(GetProperty("MessageLeaving"), "Sounds/UI/ui_notification", AllowEscape: true, DialogResult.No) != 0)
				{
					return false;
				}
			}
		...
		public void SpawnNephilim(string Blueprint = null)
		{
			...
			DelimitedStringEnumerator delimitedStringEnumerator = property.DelimitedBy(',');
			DelimitedStringEnumerator enumerator = delimitedStringEnumerator.GetEnumerator();
			while (enumerator.MoveNext())
			{
			...
			delimitedStringEnumerator = property.DelimitedBy(',');
			enumerator = delimitedStringEnumerator.GetEnumerator();
			while (enumerator.MoveNext())
			{
		...
		public static void WishTestBattle()
		{
			...
			if (!game.HasFinishedQuest("The Golem"))
			{
				...
				{
					MetricsManager.LogException("OmonporchBattle:Start", x2);
				}
			}
			...
			if (!game.HasQuest("Reclamation"))
			{
				...
				GameObject @object = GameObject.Create("DromadTrader8", 0, 0, null, delegate(GameObject x)
				{
					x.RemovePart(typeof(DromadCaravan));
				});
				zone.GetCell(49, 5).getClosestEmptyCell().AddObject(@object);
				game.StartQuest("Reclamation");
				GameObject gameObject = zone.GetCell(39, 12).AddObject("RelicChest");
				for (int i = 1; i <= 8; i++)
				{
					gameObject.ReceiveObject(RelicGenerator.GenerateRelic(i, RandomName: true));
				}
