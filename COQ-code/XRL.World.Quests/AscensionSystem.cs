...
namespace XRL.World.Quests
{
	...
	public class AscensionSystem : IQuestSystem
	{
		...
		{
			if (E.Query == "AscendQueryGolem" && E.Subject.TryGetPart<Interior>(out var Part) && !GameObject.Validate(Part.Zone.FindObject("Barathrum")) && Popup.ShowYesNo("Barathrum is not in the golem. Ascend anyway?", "Sounds/UI/ui_notification", AllowEscape: true, DialogResult.No) != 0)
			{
				E.Result = false;
				return false;
			}
		...
		public void Arrival()
		{
			...
			if (!DISABLE_ANIMATIONS)
			{
				...
				AmbientSoundsSystem.PlayAmbientBeds(zone, null, null, null, null);
				FadeToBlack.FadeIn(3f);
			...
			{
				interior.MarkActive();
				interior.ParentObject.SystemMoveTo(zone.GetCell(selected.CurrentCell.X, selected.CurrentCell.Y - 1), null);
				The.Player.SystemMoveTo(zone.GetCell(selected.CurrentCell.X, selected.CurrentCell.Y - 2), null);
				...
				if (barathrumAvailable)
				{
					barathrum.SystemMoveTo(zone.GetCell(selected.CurrentCell.X + 1, selected.CurrentCell.Y - 2), null);
				}
			}
		}
		...
		public void BarathrumStartConversation(GameObject Object)
		{
			...
			if (!currentCell.IsAdjacentTo(playerCell))
			{
				...
				{
					int i = 0;
					for (int num = findPath.Directions.Count - 1; i < num && Object.Move(findPath.Directions[i], Forced: false, System: false, IgnoreGravity: false, NoStack: false, AllowDashing: true, DoConfirmations: true, null, null, NearestAvailable: false, 0, null, null); i++)
					{
			...
			{
				Part.AttemptConversation(Silent: true, null);
				...
				Popup.Show(Object.Does("have", int.MaxValue, null, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, WithIndefiniteArticle: false, null, IndicateHidden: false, Pronoun: false, SecondPerson: true, null) + " left your party.");
			}
		}
		...
		public static void WishStart()
		{
			...
			body.SystemMoveTo(zone.GetPullDownLocation(body), null);
