...
public class GameManager : BaseGameController<GameManager>
{
	...
	public override void OnUpdate()
	{
			...
			lock (GameViewStack)
			{
				lock (TextConsole.BufferCS)
				{
					...
					for (int i = 0; i < 80; i++)
					{
						for (int j = 0; j < 25; j++)
						{
							ConsoleChar consoleChar = TextConsole.CurrentBuffer.Buffer[i, j];
							ex3DSprite2 ex3DSprite3 = ConsoleCharacter[i, j];
							consoleChar.BeforeRender(i, j, ex3DSprite3, TextConsole.CurrentBuffer);
							...
							if (consoleChar.Char == '\0')
							{
								...
								if (CurrentTile[i, j] != tile || !ColorMatch(ex3DSprite3.color, foreground) || !ColorMatch(ex3DSprite3.backcolor, color) || !ColorMatch(ex3DSprite3.detailcolor, detail))
								{
							...
							consoleChar.AfterRender(i, j, ex3DSprite3, TextConsole.CurrentBuffer);
						}
					}
					for (int k = 0; k < Backdrops.Count; k++)
					{
						if (!RequestedBackdrops.Contains(Backdrops[k].name))
						{
							Backdrops[k].SetActive(value: false);
						}
					}
					UpdateSelectedAbility();
					...
					for (int l = 0; l < 82; l++)
					{
						for (int m = 0; m < 27; m++)
						{
							if (l == 0 || l == 81 || m == 0 || m == 26)
							{
								OverlayCharacter[l, m].color = new Color(0.05f, 0.2f, 0.2f);
								OverlayCharacter[l, m].detailcolor = new Color(0.05f, 0.2f, 0.2f);
								OverlayCharacter[l, m].backcolor = new Color(0.05f, 0.23f, 0.23f, 1f);
							}
						}
					}
				}
		...
		else
		{
			...
			else if (!NeedsArrowBorder || Options.GetOption("OptionDisplayMousableZoneTransitionBorder") != "Yes")
			{
				MainCameraLetterbox.tilesWide = 80f;
				MainCameraLetterbox.tilesHigh = 25f;
				if (OverlayRoot.activeInHierarchy)
