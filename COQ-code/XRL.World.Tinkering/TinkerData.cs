...
namespace XRL.World.Tinkering
{
	...
	public sealed class TinkerData : IComposite
	{
		...
		public static bool DataDisk(string blueprint)
		{
			...
			GameObject @object = createDataDisk(TinkerRecipes.FirstOrDefault((TinkerData t) => t.Blueprint.ToLower() == blueprint));
			The.Player.ReceiveObject(@object);
			return true;
		}
		...
		public static bool DataDisk()
		{
			...
			GameObject @object = createDataDisk(TinkerRecipes[index]);
			The.Player.ReceiveObject(@object);
			return true;
		}
