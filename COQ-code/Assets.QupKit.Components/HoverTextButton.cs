...
namespace Assets.QupKit.Components
{
	public class HoverTextButton : MonoBehaviour, IPointerEnterHandler, IEventSystemHandler, IPointerExitHandler
	{
		...
		public void Update()
		{
			if (parentButton != null && parentButton.interactable && Hotkey != 0 && Input.GetKeyDown(Hotkey) && HotkeyCapitalized == (GameManager.capslock || Input.GetKey(UnityEngine.KeyCode.LeftShift) || Input.GetKey(UnityEngine.KeyCode.RightShift)) && UIManager.instance.AllowPassthroughInput())
			{
				Keyboard.ClearInput();
