...
namespace XRL.World.WorldBuilders
{
	public class JoppaWorldBuilder : WorldBuilder
	{
				...
				for (int i = 0; i < 25; i++)
				{
					for (int j = 0; j < 80; j++)
					{
						...
						if (firstObjectWithPart != null)
						{
							...
							{
								MetricsManager.LogError($"Terrain object {firstObjectWithPart.Blueprint} is missing the TerrainTravel part in world map cell [{j},{i}]");
							}
						}
						else
						{
							MetricsManager.LogError($"Missing Terrain object in world map cell [{j},{i}]");
							value = 0;
						}
						...
						int key = num;
						terrainTypes.Add(Location2D.Get(j, i), text);
						terrainComponents.Add(Location2D.Get(j, i), terrainTravel);
						if (!worldInfo.terrainLocations.ContainsKey(text))
						{
							worldInfo.terrainLocations.Add(text, new List<Location2D>());
						}
						worldInfo.terrainLocations[text].Add(Location2D.Get(j, i));
						if (!worldInfo.tierLocations.ContainsKey(key))
						{
							worldInfo.tierLocations.Add(key, new List<Location2D>());
						}
						worldInfo.tierLocations[key].Add(Location2D.Get(j, i));
						for (int k = 0; k < 3; k++)
						{
							for (int l = 0; l < 3; l++)
							{
							...
							for (int num3 = 0; num3 < 3; num3++)
							{
								...
								if (3.in100())
								{
									if (5.in100())
									{
										...
										{
											The.ZoneManager.GetZone("JoppaWorld").GetCell(m, n).GetObjectInCell(0)
												.Render.RenderString = "#";
										}
			...
			foreach (KeyValuePair<string, List<Location2D>> item in worldInfo.terrainLocations.Where((KeyValuePair<string, List<Location2D>> l) => l.Value.Count > 0).ToList().Shuffle())
			{
				foreach (Location2D item2 in item.Value.Shuffle())
				{
					...
					for (int i = 0; i <= 2; i++)
					{
						for (int j = 0; j <= 2; j++)
						{
							if ((!centerOnly || (i == 1 && j == 1)) && mutableMap.GetMutable(item2.X * 3 + i, item2.Y * 3 + j) > 0)
							{
								Location2D location2D = Location2D.Get(item2.X * 3 + i, item2.Y * 3 + j);
								if (where == null || where(location2D))
								{
									list.Add(location2D);
								}
		...
		public void AddVillages()
		{
			...
			{
				item.events.Find((HistoricEvent x) => x is InitializeVillage);
		...
		public void AddSultanHistoryLocations()
		{
			...
			{
				HistoricEntity periodSultan = QudHistoryHelpers.GetPeriodSultan(sultanHistory, num7);
				string sultanFactionName = Faction.GetSultanFactionName(num7);
				...
				faction2.Emblem.Tile = SultanShrine.GetStatueForSultan(property3, num7);
		...
		public void BuildForts(string WorldID)
		{
			...
			AddMutableEncounterToTerrain("DesertCanyon", 25, delegate(string zoneID, Location2D location, TerrainTravel pTravel)
			{
				...
				string secretID2 = AddSecret(zoneID, "a snapjaw fort", new string[3] { "snapjaw", "settlement", "humanoid" }, "Settlements");
				AddLocationFinder(zoneID, secretID2);
				GeneratedLocationInfo generatedLocationInfo = new GeneratedLocationInfo
				{
					name = "a snapjaw fort",
					targetZone = zoneID,
			...
			AddMutableEncounterToTerrain("DesertCanyon", 25, delegate(string zoneID, Location2D location, TerrainTravel pTravel)
			{
				...
				string secretID3 = AddSecret(zoneID, name, new string[3] { "apple", "settlement", "humanoid" }, "Settlements");
				AddLocationFinder(zoneID, secretID3);
			...
			AddMutableEncounterToTerrain("DesertCanyon", 25, delegate(string zoneID, Location2D location, TerrainTravel pTravel)
			{
				...
				string name2 = SettlementNames.GeneratePigFarmName(The.Game.sultanHistory);
				The.ZoneManager.SetZoneName(zoneID, name2, null, null, null, null, Proper: true);
				string secretID4 = AddSecret(zoneID, name2, new string[3] { "pig", "settlement", "humanoid" }, "Settlements");
				AddLocationFinder(zoneID, secretID4);
				The.ZoneManager.AddZonePostBuilder(zoneID, "IsCheckpoint", "Key", zoneID);
				...
				worldInfo.friendlySettlements.Add(generatedLocationInfo3);
				mutableMap.SetMutable(generatedLocationInfo3.zoneLocation, 0);
			});
		}
		...
		public void AddShugruithLair()
		{
			...
			string[] list = new string[4] { "n", "s", "e", "w" };
			string text3 = "d";
			...
			while (zp < num)
			{
				...
				The.ZoneManager.AddZonePostBuilder(ZoneIDFromXYz("JoppaWorld", location2D.X, location2D.Y, zp), "ShugBurrowBuilder");
				text3 = ((Stat.Random(1, 100) > 50 && zp != num - 1) ? list.GetRandomElement() : "d");
				while (true)
				{
					if (text3 == "d")
					{
		...
		public void BuildSecrets(string WorldID)
		{
			...
			foreach (string objectBlueprint in array)
			{
				...
				for (int num5 = 0; num5 < num4; num5++)
				{
					...
					AddSecret(text, gameObject.an(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: false, Short: false, BaseOnly: false, IndicateHidden: false, SecondPerson: true, Reflexive: false, null), new string[3]
					{
						"weep",
						text2,
