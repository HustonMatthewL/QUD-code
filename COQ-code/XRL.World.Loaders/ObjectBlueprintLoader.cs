	...
	public class ObjectBlueprintLoader
	{
			...
			public static ObjectBlueprintXMLChildNode ReadChildNode(XmlTextReader reader)
			{
				...
				if (reader.HasAttributes)
				{
					...
					if (value != null)
					{
						...
						{
							string text2 = value2(text);
							if (text2 != text)
							{
								handleWarning(new Exception($"File: {reader.BaseURI}, Line: {reader.LineNumber}:{reader.LinePosition} Compat Warning: {objectBlueprintXMLChildNode.NodeName} Name \"{text}\" has been renamed \"{text2}\"."));
								text = text2;
							}
						}
						partReflectionCache = GamePartBlueprint.PartReflectionCache.Get(ModManager.ResolveType(value, text));
						if (partReflectionCache == null)
						{
							handleError(new Exception($"File: {reader.BaseURI}, Line: {reader.LineNumber}:{reader.LinePosition} Could not find {value}.{text}, element ignored."));
							return objectBlueprintXMLChildNode;
						}
					}
					...
					do
					{
						if (partReflectionCache != null)
						{
							if (typeof(IPart).IsAssignableFrom(partReflectionCache.T) && reader.Name == "Builder")
							{
								...
								{
									handleError(new Exception($"File: {reader.BaseURI}, Line: {reader.LineNumber}:{reader.LinePosition} Could not find IPartBuilder {reader.Value}."));
									continue;
								}
							}
							else if (reader.Name != "Name" && reader.Name != "ChanceIn10000" && reader.Name != "ChanceOneIn")
							{
								...
								{
									handleError(new Exception($"File: {reader.BaseURI}, Line: {reader.LineNumber}:{reader.LinePosition} {reader.Name} is obsolete: {obsoleteAttribute.Message}"));
								}
								if (type == null)
								{
									handleError(new MissingMemberException($"File: {reader.BaseURI}, Line: {reader.LineNumber}:{reader.LinePosition} No {partReflectionCache.T.FullName}.{reader.Name} property exists."));
									continue;
								}
								try
								{
								...
								{
									handleError(new Exception($"File: {reader.BaseURI}, Line: {reader.LineNumber}:{reader.LinePosition} Error parsing {reader.Name}=\"{reader.Value}\" as {type}", innerException));
								}
							}
						}
						string str = ((reader.Name == "Name") ? text : null) ?? reader.Value;
		...
		private ObjectBlueprintXMLData Bake(ObjectBlueprintXMLData obj)
		{
			...
			if (obj.Children.TryGetValue("mixin", out var value2))
			{
				...
				{
					Mixin mixin = default(Mixin);
					mixin.Name = item2.Key;
					mixin.Include = item2.Value.Attributes?.GetValue("Include");
					mixin.Exclude = item2.Value.Attributes?.GetValue("Exclude");
					Mixin item = mixin;
		...
		public int ReadObjectsNode(XmlTextReader reader, ModInfo modInfo = null)
		{
			...
			while (reader.Read())
			{
				if (reader.Name == "object")
				{
					...
					{
						if (!Objects.TryGetValue(objectBlueprintXMLData.Name, out var value))
						{
							handleError($"File: {reader.BaseURI}, Line: {reader.LineNumber}:{reader.LinePosition} Attempt to merge with {objectBlueprintXMLData.Name} which is an unknown blueprint, node discarded");
						}
						else
						{
					...
					{
						if (Objects.TryGetValue(objectBlueprintXMLData.Name, out var value2))
						{
