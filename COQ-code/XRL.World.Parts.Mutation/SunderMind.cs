...
namespace XRL.World.Parts.Mutation
{
	...
	public class SunderMind : BaseMutation
	{
		...
		{
			if (E.Distance <= 80 && IsMyActivatedAbilityAIUsable(ActivatedAbilityID) && activeRounds <= 0 && GameObject.Validate(E.Target) && (!E.Target.HasPart<SunderMind>() || E.Target.GetPart<SunderMind>().activeRounds <= 0) && !E.Target.HasEffect<MemberOfPsychicBattle>() && E.Target.Brain != null && !E.Target.HasPart<MentalShield>() && (E.Actor.HasLOSTo(E.Target, IncludeSolid: false) || (E.Actor.HasPart<SensePsychic>() && E.Distance < E.Actor.GetPart<SensePsychic>().Radius)) && (!RealityDistortionBased || (CheckMyRealityDistortionAdvisability(null, null, null, null, null, null) && IComponent<GameObject>.CheckRealityDistortionAdvisability(E.Target, null, E.Actor, null, this, null))))
			{
		...
		public void CancelSunder()
		{
			...
			{
				IComponent<GameObject>.AddPlayerMessage("Your concentration slips and the channel between you and " + activeTarget.Object.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + " dissipates into aether.");
			}
		...
		public void EndSunder()
		{
			...
			{
				GameObject @object = activeTarget.Object;
				if (@object != null)
				{
					MemberOfPsychicBattle effect = @object.GetEffect<MemberOfPsychicBattle>();
					if (effect != null && !effect.isAttacker)
					{
						@object.RemoveEffect<MemberOfPsychicBattle>();
					}
				}
		...
		public void BeginSunder(GameObject target)
		{
			...
			if (target.ApplyEffect(new MemberOfPsychicBattle(ParentObject, isAttacker: false)))
			{
				...
				{
					IComponent<GameObject>.AddPlayerMessage("You burrow a channel through the psychic aether to " + target.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + " and begin to sunder " + target.its + " mind!");
				}
				else if (target.IsPlayer())
				{
					Popup.Show(ParentObject.T(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + " " + The.Player.DescribeDirectionToward(ParentObject) + ParentObject.GetVerb("burrow") + " a channel through the psychic aether and" + ParentObject.GetVerb("begin") + " to sunder your mind!");
		...
		public void Tick()
		{
			...
			if (effect != null)
			{
				...
				GameObject @object = activeTarget.Object;
				if (@object == null || @object.IsInvalid())
				{
					CancelSunder();
					return;
				}
				if (!@object.InSameZone(ParentObject))
				{
					if (!@object.IsValid())
					{
						EndSunder();
				...
				if (effect2 != null)
				{
					...
					Nosebleed(@object, effect2);
					if (activeRounds > 1)
					{
						ParentObject.PlayWorldSound("Sounds/Abilities/sfx_ability_sunderMind_dig");
						@object.PlayWorldSound("Sounds/Abilities/sfx_ability_sunderMind_dig");
						if (ParentObject.IsPlayer() || Visible())
						{
							ParentObject.ParticleBlip("&Y~", (10 - activeRounds) / 2, 0L);
						}
						PerformMentalAttack(Blast, ParentObject, @object, null, "SunderMind Blast", null, 8388609, 100, int.MinValue, Math.Max(ParentObject.StatMod("Ego"), base.Level - 2));
					}
					else
					{
						...
						if (Options.SifrahPsychicCombat)
						{
							if (ParentObject.IsPlayer())
							{
								PsychicCombatSifrah psychicCombatSifrah = new PsychicCombatSifrah(@object, "SunderMind", base.Level, @object.Stat("Level") / 5 + @object.StatMod("Willpower"), "sundering the mind of " + @object.an(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: false, Short: true, BaseOnly: false, IndicateHidden: false, SecondPerson: true, Reflexive: false, null));
								psychicCombatSifrah.Play(@object);
								num = num * (psychicCombatSifrah.Performance + 100) / 100;
							}
							else if (@object.IsPlayer())
							{
								PsychicCombatSifrah psychicCombatSifrah2 = new PsychicCombatSifrah(ParentObject, "SunderMind", ParentObject.Stat("Level") / 5 + ParentObject.StatMod("Ego"), base.Level, "defending against having your mind sundered by " + ParentObject.an(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: false, Short: true, BaseOnly: false, IndicateHidden: false, SecondPerson: true, Reflexive: false, null));
								psychicCombatSifrah2.Play(ParentObject);
								num = num * (100 - psychicCombatSifrah2.Performance) / 100;
							}
						}
						if (num > @object.hitpoints)
						{
							ParentObject.PlayWorldSound("Sounds/Abilities/sfx_ability_sunderMind_headExplode");
							@object.PlayWorldSound("Sounds/Abilities/sfx_ability_sunderMind_headExplode");
							if (@object.IsPlayer())
							{
							...
							{
								IComponent<GameObject>.AddPlayerMessage(@object.Poss("head") + " explodes!");
							}
							@object.BigBloodsplatter();
						}
						if (num > 0)
						{
							ParentObject.PlayWorldSound("Sounds/Abilities/sfx_ability_sunderMind_final");
							@object.PlayWorldSound("Sounds/Abilities/sfx_ability_sunderMind_final");
							@object.TakeDamage(num, "from the cumulative trauma of %t mental assault!", "Mental Psionic Unavoidable", "Your head was exploded by " + ParentObject.an(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: false, Short: true, BaseOnly: false, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + ".", @object.Its + " head was exploded by " + ParentObject.an(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: false, Short: true, BaseOnly: false, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + ".", Attacker: ParentObject, Owner: ParentObject, Source: ParentObject);
						}
					}
					activeRounds--;
					if (activeRounds <= 0)
		...
		public bool Blast(MentalAttackEvent E)
		{
			...
			@event.SetFlag("NoDamageMessage", State: true);
			...
			if (E.IsPlayerInvolved())
			{
				@event.SetParameter("Message", Event.NewStringBuilder("{{").Append(ColorCoding.ConsequentialColorChar(E.Attacker, E.Defender)).Append('|')
					.Append(E.Attacker.IsPlayer() ? "You" : E.Attacker.T(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null))
					.Append(' ')
