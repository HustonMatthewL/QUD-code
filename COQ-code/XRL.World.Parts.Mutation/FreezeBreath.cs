	...
	public class FreezeBreath : BaseMutation
	{
		...
		public void Freeze(Cell C, ScreenBuffer Buffer)
		{
			...
			if (C != null)
			{
				...
				{
					if (item.PhaseMatches(ParentObject) && item.TemperatureChange(-20 - 60 * base.Level, ParentObject, Radiant: false, MinAmbient: false, MaxAmbient: false, IgnoreResistance: false, 0, null, null))
					{
						for (int i = 0; i < 5; i++)
						{
		...
		public override bool FireEvent(Event E)
		{
			if (E.ID == "CommandFreezeBreath")
			{
				...
				for (int num = Math.Min(list.Count, Range); i < num; i++)
				{
					...
					if (i < num - 1 && list[i].IsSolidForProjectile(Projectile, ParentObject, null, ParentObject.Target, null, null))
					{
						break;
					}
				}
