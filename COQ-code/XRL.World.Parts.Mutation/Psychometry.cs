...
namespace XRL.World.Parts.Mutation
{
	...
	public class Psychometry : BaseMutation
	{
			...
			if (E.Command == COMMAND_NAME)
			{
				...
				else
				{
					if (Options.SifrahReverseEngineer && part2 != null && part2.CanDisassemble)
					{
						...
						{
							Popup.ShowFail("You must disassemble " + E.Item.t(int.MaxValue, null, null, AsIfKnown: false, Single: true, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + " in order to unlock " + E.Item.its + " secrets.");
						}
						else
						{
							Popup.ShowFail("You must learn the way of the Reverse Engineer and disassemble " + E.Item.t(int.MaxValue, null, null, AsIfKnown: false, Single: true, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + " in order to unlock " + E.Item.its + " secrets.");
						}
						return true;
					}
					...
					foreach (TinkerData tinkerRecipe in TinkerData.TinkerRecipes)
					{
						if (tinkerRecipe.Blueprint == text)
						{
							...
							try
							{
								E.Actor.PlayWorldOrUISound("sfx_characterMod_tinkerSchematic_learn", null);
		...
		public bool Advisable(GameObject Subject)
		{
			...
			if (!IComponent<GameObject>.CheckRealityDistortionAdvisability(ParentObject, Subject.GetCurrentCell() ?? ParentObject.GetCurrentCell(), ParentObject, null, this, null))
			{
				return false;
			}
			return true;
		...
		public bool Activate(GameObject Subject)
		{
			...
			{
				return ParentObject.Fail("You strain to part the veil of time in order to use psychometry on " + Subject.t(int.MaxValue, null, null, AsIfKnown: false, Single: true, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + ", but you are too confused.");
			}
			if (!IsMyActivatedAbilityVoluntarilyUsable(ActivatedAbilityID))
			{
				if (IsMyActivatedAbilityCoolingDown(ActivatedAbilityID))
				{
					ParentObject.Fail("You strain to part the veil of time in order to use psychometry on " + Subject.t(int.MaxValue, null, null, AsIfKnown: false, Single: true, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + ", but your psyche is too exhausted.");
				}
				return false;
			}
			if (!RealityDistortionBased)
			...
			@event.SetParameter("Purpose", "use psychometry on " + Subject.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null));
