	...
	public class Skittish : BaseMutation
	{
		...
		public bool LoseControl()
		{
			if (ParentObject.Target != null)
			{
				...
				List<AICommandList> @for = AIGetOffensiveAbilityListEvent.GetFor(ParentObject);
				@for.AddRange(AIGetPassiveAbilityListEvent.GetFor(ParentObject));
				@for.AddRange(AIGetMovementAbilityListEvent.GetFor(ParentObject));
				if (@for.Count > 0)
				{
					ParentObject.SetStringProperty("Skittishing", "1");
					try
					{
						if (AICommandList.HandleCommandList(@for, ParentObject, ParentObject.Target) && ParentObject.IsPlayer())
						{
