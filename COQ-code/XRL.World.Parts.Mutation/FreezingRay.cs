	...
	public class FreezingRay : BaseDefaultEquipmentMutation
	{
		...
		public void Freeze(Cell C, ref ScreenBuffer Buffer)
		{
			...
			if (C != null)
			{
				...
				{
					if (item.PhaseMatches(ParentObject) && item.TemperatureChange(-120 - 7 * base.Level, ParentObject, Radiant: false, MinAmbient: false, MaxAmbient: false, IgnoreResistance: false, 0, null, null))
					{
						for (int i = 0; i < 5; i++)
						{
		...
		public static bool Cast(FreezingRay mutation = null, string level = "5-6")
		{
			...
			for (int num = Math.Min(list.Count, 10); i < num; i++)
			{
				...
				if (i < num - 1 && list[i].IsSolidForProjectile(Projectile, mutation.ParentObject, null, mutation.ParentObject.Target, null, null))
				{
					break;
				}
			}
		...
		public override bool FireEvent(Event E)
		{
			if (E.ID == "AttackerHit")
			{
				...
				if (gameObjectParameter != null)
				{
					...
					{
						gameObjectParameter.TemperatureChange(coolOnHitAmount.RollCached(), E.GetGameObjectParameter("Attacker"), Radiant: false, MinAmbient: false, MaxAmbient: false, IgnoreResistance: false, ParentObject.GetPhase(), null, null);
					}
				}
			}
