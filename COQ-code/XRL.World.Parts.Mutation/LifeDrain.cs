...
namespace XRL.World.Parts.Mutation
{
	...
	public class LifeDrain : BaseMutation
	{
		...
		{
			if (E.Distance <= 1 && IsMyActivatedAbilityAIUsable(ActivatedAbilityID) && IsValidTarget(E.Target) && (!RealityDistortionBased || (CheckMyRealityDistortionAdvisability(null, null, null, null, null, null) && IComponent<GameObject>.CheckRealityDistortionAdvisability(E.Target, null, E.Actor, null, this, null))))
			{
			...
			if (E.ID == "CommandLifeDrain")
			{
				...
				if (cell != null)
				{
					...
					if (!IsValidTarget(combatTarget))
					{
						if (ParentObject.IsPlayer())
						{
							Popup.ShowFail("You cannot syphon vim from " + combatTarget.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + ".");
						}
						return false;
					}
