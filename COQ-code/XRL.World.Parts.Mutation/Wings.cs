				...
				if (IsMyActivatedAbilityToggledOn(FlightActivatedAbilityID))
				{
					if (ParentObject.IsPlayer() && base.currentCell != null && ParentObject.GetEffectCount(typeof(Flying)) <= 1)
					{
						...
						for (int count = base.currentCell.Objects.Count; i < count; i++)
						{
							...
							if (part != null && part.IsLongFall() && Popup.WarnYesNo("It looks like a long way down " + gameObject.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + " you're above. Are you sure you want to stop flying?") != 0)
							{
								return false;
							}
						}
