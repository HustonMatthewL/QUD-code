	...
	public class ConversationUI : IWantsTextConsoleInit
	{
		...
		public static bool Input()
		{
			...
			if (keys == Keys.MouseEvent && Keyboard.CurrentMouseEvent.Event != null)
			{
				...
				if (@event == "Command:CmdStartTrade" && Trade.Visible)
				{
					Trade.ShowScreen();
				}
			}
		...
		private static void InternalConversation()
		{
			...
			if (StartNode == null || !StartNode.Enter() || !BeginConversationEvent.Check(Listener, Speaker, Transmitter, Receiver, CurrentConversation, StartNode, Trade.Visible, Physical, Mental) || Speaker == null || !Speaker.FireEvent("ObjectTalking"))
			{
				return;
			}
		...
		public static void CheckLost()
		{
			...
			else if (Speaker.TryGetEffect<Lost>(out Effect) && CanGiveDirectionsEvent.Check(Speaker, Listener, Transmitter, Receiver, CurrentConversation, Trade.Visible, Physical, Mental))
			{
				...
				Popup.Show(Speaker.Does("ask", int.MaxValue, null, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: true, WithoutTitles: true, Short: true, BaseOnly: false, WithIndefiniteArticle: false, null, IndicateHidden: false, Pronoun: false, SecondPerson: true, null) + " about " + Speaker.its + " location and " + Speaker.GetVerb("are") + " no longer lost.");
		...
		public static void Render()
		{
			...
			int num = Popup.ShowConversation(GetTitle(), gameObject, CurrentNode.GetDisplayText(WithColor: true), options, Trade.Visible, CurrentNode.AllowEscape, CurrentConversation?.HasState("RenderMapBehind") ?? false);
			switch (num)
			{
			case -1:
				Escape();
