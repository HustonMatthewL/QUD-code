	...
	public class InventoryScreen : IScreen, IWantsTextConsoleInit
	{
		...
		public static void RebuildLists(GameObject GO)
		{
			...
			for (int i = 0; i < objectsDirect.Count; i++)
			{
				...
				if (filterString != "" && !gameObject.GetDisplayName(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: true, ColorOnly: false, Visible: true, WithoutTitles: false, ForSort: false, Short: false, BaseOnly: false, WithIndefiniteArticle: false, WithDefiniteArticle: false, null, IndicateHidden: false, Capitalize: false, SecondPerson: false, Reflexive: false, null).Contains(filterString, CompareOptions.IgnoreCase))
				{
				...
				while (true)
				{
					...
					while (true)
					{
						...
						if (Selected < num)
						{
							...
							{
								filterString = Popup.AskString("Enter text to filter inventory by item name.", filterString, "Sounds/UI/ui_notification", null, null, 80, 0, ReturnNullForEscape: false, EscapeNonMarkupFormatting: true, null);
								ClearLists();
							}
