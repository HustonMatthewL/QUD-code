		...
		public ScreenReturn Show(GameObject GO)
		{
			...
			while (!flag)
			{
				...
				if (!headerMessage.IsNullOrEmpty())
				{
					...
					for (int i = 0; i < textBlock.Lines.Count; i++)
					{
						scrapBuffer.WriteAt(3, num6++, textBlock.Lines[i]);
					}
					num6++;
				}
				...
				num6 += 2;
				num2 = 23 - num6;
				for (int j = 0; j + tbStart < TB.Lines.Count && j < num2; j++)
				{
					scrapBuffer.Goto(50, num6 + j);
					scrapBuffer.Write(TB.Lines[j + tbStart]);
				}
				if (TB.Lines.Count > num2)
				{
					int num7 = (int)((float)num2 / (float)TB.Lines.Count * 23f);
					int num8 = (int)((float)tbStart / (float)TB.Lines.Count * 23f);
				...
				nDisplayed = 23 - num6;
				for (int k = topRow; k < topRow + nDisplayed && k < factionsByName.Count; k++)
				{
					int num9 = ((k != cursorRow) ? WriteFaction(scrapBuffer, GO, factionsByName[k], 0, 3, num6) : WriteFaction(scrapBuffer, GO, factionsByName[k], 1 + num, 3, num6));
					num6 += num9;
					if (num9 > 0)
					{
						nDisplayed -= num9 - 1;
