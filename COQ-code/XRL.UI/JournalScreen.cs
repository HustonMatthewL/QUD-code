...
namespace XRL.UI
{
	public class JournalScreen : IScreen
	{
		...
		public static List<IBaseJournalEntry> GetRawEntriesFor(string Tab, string Category = null)
		{
			...
			else if (Tab == STR_LOCATIONS)
			{
				LastRaw.AddRange(from c in JournalAPI.MapNotes
					where c.Revealed && (c.Category == Category || Category == null)
					select c into x
		...
		public static bool HandleInsert(string selectedTab, GameObject GO)
		{
			...
			{
				string text = Popup.AskString("Entry text", "", "Sounds/UI/ui_notification", null, null, 2147483646, 0, ReturnNullForEscape: false, EscapeNonMarkupFormatting: true, null);
			...
			while (!flag)
			{
				...
				if ((keys == Keys.N || (keys == Keys.MouseEvent && Keyboard.CurrentMouseEvent.Event == "Command:V Positive")) && array[num] == STR_LOCATIONS)
				{
					...
					else
					{
						...
						string text2 = Popup.AskString(namedByPlayer ? ("Enter a new name for " + baseDisplayName + ".") : "Enter a name for this location.", "", "Sounds/UI/ui_notification", null, null, 40, 0, ReturnNullForEscape: false, EscapeNonMarkupFormatting: true, null);
