	...
	public class SkillsAndPowersScreen : BaseScreen, IScreen
	{
		...
		public static void SelectNode(SPNode node, GameObject GO)
		{
			...
			else if (!GO.HasSkill(node.ParentNode.Skill.Class))
			{
				...
				@class = node.ParentNode.Skill.Class;
				text = "skill";
			...
			if (GO.HasPart(@class))
			{
				Popup.Show("You already have that " + text + ".");
			}
			else if ((powerEntry != null && powerEntry.IsSkillInitiatory) || (skillEntry != null && skillEntry.Initiatory))
			{
				Popup.Show("You must be initiated into this " + text + " in order to learn it.");
			}
			else if (GO.Stat("SP") < cost)
			{
				Popup.Show("You don't have enough skill points to buy that " + text + "!");
			}
			else
			{
				...
				string text2 = "XRL.World.Parts.Skill." + @class;
				Type type = ModManager.ResolveType(text2);
				if (type == null)
				{
					Popup.Show("No implementation for " + text2);
					return;
				}
				if (text != "power")
				{
								...
								else
								{
									if (power.Requires != null)
									{
										foreach (string item in power.Requires.CachedCommaExpansion())
										{
											...
											{
												text4 = MutationFactory.GetMutationEntryByName(item).DisplayName;
									...
									if (power.Exclusion != null)
									{
										foreach (string item2 in power.Exclusion.CachedCommaExpansion())
										{
											...
											{
												text5 = MutationFactory.GetMutationEntryByName(item2).DisplayName;
