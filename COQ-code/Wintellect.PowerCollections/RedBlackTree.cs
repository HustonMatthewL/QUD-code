...
namespace Wintellect.PowerCollections
{
	...
	internal class RedBlackTree<T> : IEnumerable<T>, IEnumerable
	{
		...
		public bool Insert(T item, DuplicatePolicy dupPolicy, out T previous)
		{
			...
			Node node4 = null;
			StopEnumerations();
			bool flag3 = dupPolicy != 0 && dupPolicy != DuplicatePolicy.InsertLast;
			Node[] array = null;
			int num = 0;
			if (flag3)
			{
		...
		public bool DeleteItemFromRange(RangeTester rangeTester, bool deleteFirst, out T item)
		{
			...
			while (true)
			{
				if ((node.left == null || !node.left.IsRed) && (node.right == null || !node.right.IsRed))
				{
					...
					else if ((node4.left == null || !node4.left.IsRed) && (node4.right == null || !node4.right.IsRed))
					{
						node.IsRed = true;
						node4.IsRed = true;
						node3.IsRed = false;
					...
					{
						if (node3.left == node && (node4.right == null || !node4.right.IsRed))
						{
							Node left = node4.left;
							Rotate(node3, node4, left);
							node4 = left;
						}
						else if (node3.right == node && (node4.left == null || !node4.left.IsRed))
						{
							Node right = node4.right;
							Rotate(node3, node4, right);
							node4 = right;
						}
						Rotate(node2, node3, node4);
						node.IsRed = true;
						node4.IsRed = true;
						node4.left.IsRed = false;
						node4.right.IsRed = false;
						node4.DecrementCount();
						nodeStack[num - 1] = node4;
				...
				if (!node3.IsRed)
				{
					...
					node3.IsRed = true;
					node2 = node4;
					node4 = ((node3.left == node) ? node3.right : node3.left);
				}
			}
		}
