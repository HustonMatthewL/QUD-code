	...
	public class DeepDream : Effect
	{
		...
		public bool Crungle()
		{
			...
			try
			{
				...
				Target.GiveProperName(null, Force: false, null, SpecialFaildown: false, null, null);
				if (!Target.ApplyEffect(new WakingDream(player, Source)))
				{
					return false;
				}
		...
		{
			The.Core.RenderBase();
			if (Object.CurrentCell.AnyLocalAdjacentCell((Cell x) => !x.IsVisible()))
			{
				...
				else if (!Object.HasPart(typeof(DarkVision)))
				{
