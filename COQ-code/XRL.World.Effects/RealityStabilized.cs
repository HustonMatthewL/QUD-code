	...
	public class RealityStabilized : Effect, ITierInitialized
	{
		...
		private void FailedToContest(GameObject GO)
		{
			...
			else
			{
				...
				{
					IComponent<GameObject>.AddPlayerMessage("You feel a psychic thud as " + (sensePsychicEffect.Identified ? GO.does("push", int.MaxValue, null, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, WithIndefiniteArticle: false, null, IndicateHidden: false, Pronoun: false, SecondPerson: true, null) : "someone pushes") + " against the structure of spacetime and " + (sensePsychicEffect.Identified ? GO.GetVerb("fail") : "fails") + " to break through.");
				}
				if (bodyPart != null && bodyPart.VariantType == "Face" && GO.GetIntProperty("Analgesia") <= 0 && IComponent<GameObject>.Visible(GO))
				{
					IComponent<GameObject>.AddPlayerMessage(GO.Does("wince", int.MaxValue, null, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, WithIndefiniteArticle: false, null, IndicateHidden: false, Pronoun: false, SecondPerson: true, null) + ".");
				}
			}
			...
			GO.UseEnergy(1000, "Effect", null, null);
		}
		...
		private ContestResult TryContest(GameObject GO, int Adjust = 0, int Performance = int.MinValue)
		{
			...
			if (!GO.IsPlayer())
			{
				...
				if (sensePsychicEffect != null)
				{
					IComponent<GameObject>.AddPlayerMessage("You feel a psychic whiff as " + (sensePsychicEffect.Identified ? GO.does("push", int.MaxValue, null, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, WithIndefiniteArticle: false, null, IndicateHidden: false, Pronoun: false, SecondPerson: true, null) : "something pushes") + " past resistance in the structure of spacetime.");
				}
			}
		...
		private static void ShortCircuitDevice(GameObject Device, GameObject User, Event FromEvent = null)
		{
			...
			if (User != null)
			{
				...
				Device.UseCharge("10d100".RollCached(), LiveOnly: false, 0L, IncludeTransient: true, IncludeBiological: true, null);
				User.UseEnergy(1000, "Item Use Failure", null, null);
			}
			...
			if (E.ID == "ObjectEnteringCellBlockedBySolid")
			{
				if (Projective)
				{
					...
					if (CanDestroy)
					{
						...
						{
							StringBuilder stringBuilder = Event.NewStringBuilder();
							stringBuilder.Append(gameObjectParameter.T(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null));
						...
						base.Object.UseEnergy(1000, "Movement Failure", null, null);
					}
				}
			}
