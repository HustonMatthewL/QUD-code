...
namespace XRL.World.Effects
{
	...
	public class CookingDomainPhase_UnitPhaseOnDamage : ProceduralCookingEffectUnit
	{
		...
		public override void FireEvent(Event E)
		{
			...
			if (@object != null && Stat.Random(1, 100) <= Tier && @object.CurrentCell != null && !@object.OnWorldMap() && !@object.HasEffect<Phased>() && IComponent<GameObject>.CheckRealityDistortionAccessibility(@object))
			{
				@object.ApplyEffect(new Phased(Stat.Random(8, 10)));
				for (int i = 0; i < 5; i++)
				{
					XRLCore.ParticleManager.AddRadial("&bù", @object.CurrentCell.X, @object.CurrentCell.Y, Stat.Random(0, 7), Stat.Random(5, 10), 0.01f * (float)Stat.Random(4, 6), -0.05f * (float)Stat.Random(3, 7));
				}
			}
		}
	}
