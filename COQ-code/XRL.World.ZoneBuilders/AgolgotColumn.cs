	...
	public class AgolgotColumn : GirshLairMakerBase
	{
		...
		public override bool BuildLair(Zone Z)
		{
			...
			if (Z.Z != CradleLevel && list2.Count > 0)
			{
				...
				int l = 0;
				for (int num8 = Stat.Random(0, 4); l < num8; l++)
				{
					...
					foreach (PathfinderNode step in pathfinder.Steps)
					{
						...
						GameObject @object = GameObject.Create(randomElement.Blueprint);
						cell2.AddObject(@object);
					}
				}
			}
			int chance = 20;
			for (int m = 0; m < Z.Height; m++)
			{
				...
				{
					LiquidVolume liquidVolume = Z.GetCell(n, m).GetOpenLiquidVolume()?.LiquidVolume;
					if (liquidVolume != null && liquidVolume.Volume > 500 && chance.in100() && liquidVolume.IsWaste() && liquidVolume.IsOpenVolume())
					{
						Z.GetCell(n, m).AddObject("EelSpawn");
					}
				}
			}
			return result;
