...
namespace XRL.World.ZoneBuilders
{
	public class ForceConnections : ZoneBuilderSandbox
	{
		...
		public bool _BuildZone(Zone Z, List<Point2D> additional = null)
		{
			...
			{
				int num = 0;
				num = (int)(Math.Abs(pathNoise.GetNoise((x + Z.wX * 80) / 3, y + Z.wY * 25)) * 160f);
				if (Z.GetCell(x, y).HasWall())
				{
					return 80 + num;
				}
				return Z.GetCell(x, y).HasObject("InfluenceMapBlocker") ? (80 + num) : num;
			}))
			{
				for (int k = 1; k < list.Count; k++)
				{
					if (Z.IsReachable(list[k].x, list[k].y) || !pathfinder.FindPath(list[0].location, list[k].location, Display: false, CardinalDirectionsOnly: true, 24300, ShuffleDirections: true))
					{
						continue;
					}
					...
					Z.BuildReachableMap(list[k].x, list[k].y, bClearFirst: false);
				}
			}
			return true;
		}
