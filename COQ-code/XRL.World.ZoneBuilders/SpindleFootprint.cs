			...
			Action<Rect2D, string> action = delegate(Rect2D r, string d)
			{
				if (mode != FootprintMode.Foundation)
				{
					...
					case "S":
					{
						...
						{
							Z.GetCell(j, (d == "N") ? r.y1 : r.y2).AddObject("SpindleGlassWall");
							Z.GetCell(j, (d == "N") ? r.y2 : r.y1).AddObject("SpindleLight");
						}
						break;
					}
					case "E":
					case "W":
					{
						...
						{
							Z.GetCell((d == "W") ? r.x1 : r.x2, i).AddObject("SpindleLight");
							Z.GetCell((d == "E") ? r.x2 : r.x1, i).AddObject("SpindleGlassWall");
						}
						break;
					}
					}
