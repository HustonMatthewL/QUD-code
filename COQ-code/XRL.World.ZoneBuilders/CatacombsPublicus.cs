...
namespace XRL.World.ZoneBuilders
{
	public class CatacombsPublicus : ZoneBuilderSandbox
	{
		public bool BuildZone(Zone Z)
		{
			...
			if (Z.X == 0 && Z.Y == 2)
			{
				for (int j = 44; j <= 45; j++)
				{
					...
					foreach (Cell cell in emptyCellsShuffled)
					{
						...
						Location2D location2 = Z.GetCell(j, 24).Location;
			...
			int k = 0;
			for (int num = Stat.Random(1, 2); k < num; k++)
			{
				list2.Add(list[0]);
				list.RemoveAt(0);
			}
			...
			if (Z.X == 0 && Z.Y == 0)
			{
				MapFile mapFile = MapFile.LoadWithMods("MopangoSettlement.rpm");
				for (int l = 0; l < 13; l++)
				{
					for (int m = 0; m < 35; m++)
					{
						...
						cell2.Clear();
						foreach (MapFileObjectBlueprint @object in mapFile.Cells[m, l].Objects)
						{
