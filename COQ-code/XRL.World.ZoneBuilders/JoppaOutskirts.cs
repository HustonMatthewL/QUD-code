...
namespace XRL.World.ZoneBuilders
{
	public class JoppaOutskirts : ZoneBuilderSandbox
	{
		public bool BuildZone(Zone Z)
		{
				...
				if (influenceMap.Regions.Count > 0)
				{
					...
					List<Cell> cells = Z.GetCells(delegate(Cell c)
					{
						...
						for (int num15 = 0; num15 < prefabHeight; num15++)
						{
							...
							{
								Cell cellFromOffset = c.GetCellFromOffset(num16, num15);
					...
					for (int num17 = 0; num17 < prefabHeight; num17++)
					{
						...
						{
							randomElement2.GetCellFromOffset(num18, num17).Clear();
						}
					}
			...
			{
				for (int num19 = 0; num19 < Z.Height; num19++)
				{
					for (int num20 = 0; num20 < Z.Width; num20++)
					{
						if (Z.GetCell(num20, num19).HasWall())
						{
							pathfinder.CurrentNavigationMap[num20, num19] = 9999;
						}
						else if (Z.GetCell(num20, num19).HasObjectWithPart("LiquidVolume"))
						{
							pathfinder.CurrentNavigationMap[num20, num19] = 200;
						}
						else
						{
							pathfinder.CurrentNavigationMap[num20, num19] = Stat.Random(0, 50);
						}
					}
				}
