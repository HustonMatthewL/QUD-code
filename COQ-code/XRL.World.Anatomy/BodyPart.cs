				...
				else if (slotsRequiredFor > 2)
				{
					...
					if (list2.Count < slotsRequiredFor - 1 && UnequipOthers)
					{
						foreach (BodyPart item6 in part3)
						{
							...
							Event event3 = Event.New("CommandUnequipObject", "BodyPart", item6);
							if (Silent)
							{
							...
							if (AutoEquipTry > 0)
							{
								event3.SetParameter("AutoEquipTry", AutoEquipTry);
							}
							if (!FailureMessage.IsNullOrEmpty())
							{
								event3.SetParameter("FailureMessage", FailureMessage);
							}
							if (ForDeepCopy)
							{
								event3.SetFlag("ForDeepCopy", State: true);
							}
							if (ParentBody.ParentObject.FireEvent(event3))
							{
							...
							{
								string stringParameter3 = event3.GetStringParameter("FailureMessage");
