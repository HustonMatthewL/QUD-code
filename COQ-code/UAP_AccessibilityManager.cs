...
public class UAP_AccessibilityManager : MonoBehaviour
{
	...
	private static bool ShouldAutoEnable()
	{
		...
		if (num == 0 && instance.m_AutoTurnOnIfScreenReaderDetected && WindowsTTS.IsScreenReaderDetected())
		{
			result = true;
		}
		return result;
	...
	public static void RecalculateUIElementsOrder(GameObject parent = null)
	{
		...
		{
			AccessibleUIGroupRoot[] componentsInChildren = UnityEngine.Object.FindObjectsOfType(typeof(AccessibleUIGroupRoot)) as AccessibleUIGroupRoot[];
			for (int i = 0; i < componentsInChildren.Length; i++)
			{
				componentsInChildren[i].RefreshContainer();
			}
