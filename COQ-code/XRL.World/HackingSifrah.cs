...
namespace XRL.World
{
	...
	public class HackingSifrah : TinkeringSifrah
	{
		...
		{
			Description = "Hacking " + ContextObject.an(int.MaxValue, null, null, AsIfKnown: false, Single: true, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: false, Short: false, BaseOnly: false, IndicateHidden: false, SecondPerson: true, Reflexive: false, null);
			...
			int @for = GetTinkeringBonusEvent.GetFor(The.Player, ContextObject, "Hacking", MaxTurns, 0, ref SecondaryBonus, ref Interrupt, ref PsychometryApplied, Interruptable: true, ForSifrah: true);
			if (Interrupt)
			{
				Finished = true;
				Abort = true;
			...
			else
			{
				...
				tokens = Tokens.Count;
				asMaster = true;
				if (GetGameCount(null, slots2, tokens, null, asMaster) >= num6)
				{
					AsMaster = true;
				}
			}
