...
namespace XRL.World
{
	...
	public class ExamineSifrah : TinkeringSifrah
	{
		...
		{
			Description = "Examining " + ContextObject.an(int.MaxValue, null, null, AsIfKnown: false, Single: true, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: false, Short: false, BaseOnly: false, IndicateHidden: false, SecondPerson: true, Reflexive: false, null);
			...
			int @for = GetTinkeringBonusEvent.GetFor(The.Player, ContextObject, "Examine", MaxTurns, 0, ref SecondaryBonus, ref Interrupt, ref PsychometryApplied, Interruptable: true, ForSifrah: true);
			if (Interrupt)
			{
				Finished = true;
				Abort = true;
			...
			else
			{
				...
				if (gameCount2 + gameCount3 / 100 >= num6)
				{
					AsMaster = true;
				}
			}
