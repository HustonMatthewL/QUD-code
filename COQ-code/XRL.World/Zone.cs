...
namespace XRL.World
{
	...
	public class Zone
	{
			...
			for (int i = 0; i < Height; i++)
			{
				for (int j = 0; j < Width; j++)
				{
					...
					else
					{
						...
						{
							GameObject @object = cell.Objects[n];
							if (AutoexploreObjectEvent.Check(The.Player, @object, AutoAct.Setting, AutoAct.Action))
							{
								flag = true;
								break;
							}
		...
		public string GetTerrainDisplayName(bool Stripped = true)
		{
			...
			return GetTerrainObject()?.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null);
		}
			...
			else
			{
				for (int i = x0; i <= x1; i++)
				{
					...
					if (i != x0 && i != x1)
					{
						...
						else if (IncludeSolid && cell.IsSolidForProjectile(null, UseTargetability, null, null, null, null) && ++num7 > PassBlocks)
						{
							return false;
						}
					}
		...
		public void Render(ScreenBuffer Buf, int X1, int Y1, int X2, int Y2)
		{
			...
			for (int k = Y1; k <= Y2; k++)
			{
				for (int l = X1; l <= X2; l++)
				{
					ConsoleChar @char = Buf.Buffer[l, k];
					Map[l][k].RenderSound(@char, Buf.Buffer);
				}
			}
