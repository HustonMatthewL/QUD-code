...
namespace XRL.World
{
	...
	public class DisarmingSifrah : TinkeringSifrah
	{
		...
		{
			Description = "Disarming " + ContextObject.an(int.MaxValue, null, null, AsIfKnown: false, Single: true, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: false, Short: false, BaseOnly: false, IndicateHidden: false, SecondPerson: true, Reflexive: false, null);
			...
			int @for = GetTinkeringBonusEvent.GetFor(The.Player, ContextObject, "Disarming", MaxTurns, 0, ref Interrupt, ref PsychometryApplied, Interruptable: true, ForSifrah: true);
			if (Interrupt)
			{
				Finished = true;
				Abort = true;
			...
			else
			{
				...
				if (gameCount2 + gameCount3 / 75 >= num6)
				{
					AsMaster = true;
				}
			}
		...
		public static bool IsMastered(GameObject ContextObject, int Difficulty, int Rating)
		{
			...
			int? slots = num2;
			int? tokens = num;
			if (SifrahGame.GetGameCount(gameType, null, slots, tokens, asMaster, null) > 0)
			{
				return true;
			}
			tokens = num2;
			slots = num;
			int gameCount = SifrahGame.GetGameCount(gameType, null, tokens, slots, null, null);
			int num4 = SifrahGame.GetGameCount(gameType, Slots: num2, Tokens: num, Outcome: GameOutcome.Failure, AsMaster: null, Mastered: null);
			int num5 = SifrahGame.GetGameCount(gameType, Slots: num2, Tokens: num, Outcome: GameOutcome.CriticalFailure, AsMaster: null, Mastered: null);
