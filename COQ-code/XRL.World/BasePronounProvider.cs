...
namespace XRL.World
{
	...
	public abstract class BasePronounProvider : IPronounProvider
	{
		...
		protected virtual async Task<bool> CustomizeProcess(string What)
		{
			...
			_Subjective = await Popup.AskStringAsync("What subjective pronoun (he, she, they, etc.) should be used for this " + What + "?", Subjective, 6, 0, "abcdefghijklmnopqrstuvwxyz", ReturnNullForEscape: false, EscapeNonMarkupFormatting: true, null);
			if (string.IsNullOrEmpty(_Subjective))
			{
				return false;
			}
			_Objective = await Popup.AskStringAsync("What objective pronoun (him, her, them, etc.) should be used for this " + What + "?", Objective, 6, 0, "abcdefghijklmnopqrstuvwxyz", ReturnNullForEscape: false, EscapeNonMarkupFormatting: true, null);
			if (string.IsNullOrEmpty(_Objective))
			{
				return false;
			}
			_PossessiveAdjective = await Popup.AskStringAsync("What possessive adjective (his, her, their, etc.) should be used for this " + What + "?", PossessiveAdjective, 6, 0, "abcdefghijklmnopqrstuvwxyz", ReturnNullForEscape: false, EscapeNonMarkupFormatting: true, null);
			if (string.IsNullOrEmpty(_PossessiveAdjective))
			{
				return false;
			}
			_SubstantivePossessive = null;
			_SubstantivePossessive = await Popup.AskStringAsync("What substantive possessive (his, hers, theirs, etc.) should be used for this " + What + "?", SubstantivePossessive, 6, 0, "abcdefghijklmnopqrstuvwxyz", ReturnNullForEscape: false, EscapeNonMarkupFormatting: true, null);
			if (string.IsNullOrEmpty(_SubstantivePossessive))
			{
				return false;
			}
			_Reflexive = null;
			_Reflexive = await Popup.AskStringAsync("What reflexive pronoun (himself, herself, themself, themselves, etc.) should be used for this " + What + "?", Reflexive, 12, 0, "abcdefghijklmnopqrstuvwxyz", ReturnNullForEscape: false, EscapeNonMarkupFormatting: true, null);
			if (string.IsNullOrEmpty(_Reflexive))
			{
				return false;
			}
			_PersonTerm = await Popup.AskStringAsync("What term should be used for a mature person of this " + What + "? (Man, woman, person, etc.)", PersonTerm, 12, 0, "abcdefghijklmnopqrstuvwxyz", ReturnNullForEscape: false, EscapeNonMarkupFormatting: true, null);
			if (string.IsNullOrEmpty(_PersonTerm))
			{
				return false;
			}
			_ImmaturePersonTerm = await Popup.AskStringAsync("What term should be used for an immature person of this " + What + "? (Boy, girl, child, etc.)", ImmaturePersonTerm, 12, 0, "abcdefghijklmnopqrstuvwxyz", ReturnNullForEscape: false, EscapeNonMarkupFormatting: true, null);
			if (string.IsNullOrEmpty(_ImmaturePersonTerm))
			{
				return false;
			}
			_FormalAddressTerm = await Popup.AskStringAsync("What term should be used to formally address a person of this " + What + "? (Sir, madam, friend, etc.)", FormalAddressTerm, 12, 0, "abcdefghijklmnopqrstuvwxyz", ReturnNullForEscape: false, EscapeNonMarkupFormatting: true, null);
			if (string.IsNullOrEmpty(_FormalAddressTerm))
			{
				return false;
			}
			_OffspringTerm = await Popup.AskStringAsync("What term should be used to address a person of this " + What + " as an offspring? (Son, daughter, child, etc.)", OffspringTerm, 12, 0, "abcdefghijklmnopqrstuvwxyz", ReturnNullForEscape: false, EscapeNonMarkupFormatting: true, null);
			if (string.IsNullOrEmpty(_OffspringTerm))
			{
				return false;
			}
			_SiblingTerm = await Popup.AskStringAsync("What term should be used to address a person of this " + What + " as a sibling? (Brother, sister, sib, etc.)", SiblingTerm, 12, 0, "abcdefghijklmnopqrstuvwxyz", ReturnNullForEscape: false, EscapeNonMarkupFormatting: true, null);
			if (string.IsNullOrEmpty(_SiblingTerm))
			{
				return false;
			}
			_ParentTerm = await Popup.AskStringAsync("What term should be used to address a person of this " + What + " as a parent? (Father, mother, progenitor, etc.)", ParentTerm, 12, 0, "abcdefghijklmnopqrstuvwxyz", ReturnNullForEscape: false, EscapeNonMarkupFormatting: true, null);
