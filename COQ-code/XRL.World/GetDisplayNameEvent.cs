	...
	public class GetDisplayNameEvent : PooledEvent<GetDisplayNameEvent>
	{
		...
		public string ProcessFor(GameObject obj, bool NoReturn = false)
		{
			...
			if (obj.HasRegisteredEvent("GetDisplayName"))
			{
				...
				@event.SetFlag("ColorOnly", ColorOnly);
				@event.SetFlag("Visible", Visible);
				@event.SetFlag("UsingAdjunctNoun", UsingAdjunctNoun);
				@event.SetFlag("WithoutTitles", WithoutTitles);
				@event.SetFlag("ForSort", ForSort);
				@event.SetFlag("Reference", Reference);
				obj.FireEvent(@event);
