...
namespace XRL.World
{
	...
	public class ZoneManager
	{
			...
			try
			{
				...
				try
				{
					using (PooledList<byte> pooledList = new PooledList<byte>(2048))
					{
						...
						try
						{
							if (!BrotliDecoder.TryDecompress(pooledList.AsSpan(4), array, out var _))
							{
		...
		public void FreezeZone(Zone Z)
		{
			...
			try
			{
				...
				try
				{
					...
					try
					{
						if (!BrotliEncoder.TryCompress(buffer.AsSpan(0, num), array.AsSpan(4, num), out var bytesWritten, 1, 16))
						{
							throw new InvalidDataException();
						}
						...
						cacheOperation.Command.Parameters.Add("Data", DbType.Binary).Value = array.AsSpan(0, bytesWritten + 4).ToArray();
				...
				while (true)
				{
					...
					if (request.IsWorldZone)
					{
						...
						if (!request.World.Map.IsNullOrEmpty())
						{
							...
							mapBuilder.FileName = request.World.Map;
			...
			try
			{
				for (int num5 = 0; num5 < Z.Height; num5++)
				{
					for (int num6 = 0; num6 < Z.Width; num6++)
					{
						...
						{
							@object.SystemMoveTo(cell6.getClosestPassableCell((Cell c) => !c.HasObjectWithPart("StairsDown")), null);
						}
					}
				}
			}
			...
			try
			{
				for (int num7 = 0; num7 < Z.Height; num7++)
				{
					for (int num8 = 0; num8 < Z.Width; num8++)
					{
						...
						{
							item5.SystemMoveTo(cell7.getClosestPassableCell((Cell c) => !c.HasObjectWithTagOrProperty("NoOverlap")), null);
						}
					}
				}
			}
		...
		private void ApplyPropertiesToZone(Dictionary<string, object> Properties, Zone Z)
		{
			...
			Dictionary<string, object> dictionary2 = ZoneProperties.GetValue(Z.ZoneID) ?? (ZoneProperties[Z.ZoneID] = new Dictionary<string, object>());
			foreach (KeyValuePair<string, object> Property in Properties)
			{
				dictionary2[Property.Key] = Property.Value;
			}
		}
