...
public class AscendAnimation : MonoBehaviour
{
	...
	private void LateUpdate()
	{
		...
		else if (Phase == 7)
		{
			...
			{
				FadeToBlack.Fade(0f, 1f, 1f, Camera.main.backgroundColor, null);
		...
		for (int i = 0; i < 3; i++)
		{
			ParticleSystem.SizeBySpeedModule sizeBySpeed = system[i].sizeBySpeed;
			sizeBySpeed.yMultiplier = Mathf.Min(100f, Speed / 10f);
			for (int j = 0; j < MaxStars; j++)
			{
				ParticleSystem.Particle[] array = Stars[i];
				array[j].position += new Vector3(0f, (0f - Speed - array[j].startSize * 0.05f) * Time.deltaTime, 0f);
				if (array[j].position.y < (float)(-Screen.height / 2))
				{
					array[j].position = new Vector3(UnityEngine.Random.Range(-Screen.width / 2, Screen.width / 2), Screen.height / 2 + UnityEngine.Random.Range(0, 1000), array[j].position.z);
				}
			}
			system[i].SetParticles(Stars[i], MaxStars);
		}
		if (TotalTimer > 60f)
		{
			The.Core.RenderBase();
