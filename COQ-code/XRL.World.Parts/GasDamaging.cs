...
namespace XRL.World.Parts
{
	...
	public class GasDamaging : IGasBehavior
	{
			...
			if (!E.IgnoreGases && (!Respiratory || !E.Unbreathing) && E.PhaseMatches(ParentObject))
			{
				...
				else if (E.Smart)
				{
					...
					if (CheckGasCanAffectEvent.Check(E.Actor, ParentObject) && Match(E.Actor))
					{
						...
						int value = (Respiratory ? GetRespiratoryAgentPerformanceEvent.GetFor(E.Actor, ParentObject, part, null) : part.Density);
		...
		public bool ApplyDamagingGasWithResult(GameObject GO)
		{
			...
			int num = (Respiratory ? GetRespiratoryAgentPerformanceEvent.GetFor(GO, ParentObject, part, null) : part.Density);
			if (num <= 0)
			{
				return false;
			}
			...
			if (ParentObject.GetDisplayName(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: true, ColorOnly: false, Visible: true, WithoutTitles: false, ForSort: false, Short: false, BaseOnly: true, WithIndefiniteArticle: false, WithDefiniteArticle: false, null, IndicateHidden: false, Capitalize: false, SecondPerson: false, Reflexive: false, null) != Noun)
			{
