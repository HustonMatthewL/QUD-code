	...
	public class Body : IPart
	{
		...
		public GameObject Dismember(BodyPart Part, GameObject Actor = null, IInventory Where = null, bool Obliterate = false, bool Silent = false, IEvent ParentEvent = null)
		{
			...
			if (!Part.Extrinsic && !Obliterate)
			{
				...
				string displayName = ParentObject.GetDisplayName(int.MaxValue, null, null, AsIfKnown: true, Single: false, NoConfusion: true, NoColor: false, Stripped: true, ColorOnly: false, Visible: true, WithoutTitles: true, ForSort: false, Short: false, BaseOnly: true, WithIndefiniteArticle: false, WithDefiniteArticle: false, null, IndicateHidden: false, Capitalize: false, SecondPerson: false, Reflexive: false, null);
				ParentObject.An(int.MaxValue, null, null, AsIfKnown: true, Single: false, NoConfusion: true, NoColor: false, Stripped: true, WithoutTitles: true, Short: true, BaseOnly: true, IndicateHidden: false, SecondPerson: false, Reflexive: false, null);
				if (!gameObject.HasPropertyOrTag("SeveredLimbKeepAppearance"))
				{
					...
					{
						string @default = ParentObject.GetxTag("Grammar", "iArticle", gameObject.GetStringProperty("OverrideIArticle"));
						@default = ParentObject.GetPropertyOrTag("OverrideIArticle", @default);
						if (!@default.IsNullOrEmpty())
						{
							gameObject.SetStringProperty("OverrideIArticle", @default);
						}
					}
					string primaryLiquidNameForLiquidSpecification = Bleeding.GetPrimaryLiquidNameForLiquidSpecification(ParentObject.GetTagOrStringProperty("BleedLiquid", "blood-1000"));
				...
				if (Part.Type == "Face")
				{
					...
					armor.Ego = ego;
					foreach (KeyValuePair<string, int> item in ParentObject.Brain.Allegiance)
					{
		...
		private List<string> SummarizeMissingBodyParts(List<BodyPart> Parts)
		{
			...
			{
				int num = Counts[n1].CompareTo(Counts[n2]);
				return (num != 0) ? (-num) : n1.CompareTo(n2);
			});
			List<string> list2 = new List<string>(list.Count);
			foreach (string item in list)
			{
				int num2 = Counts[item];
				if (num2 == 1)
				{
				...
				{
					list2.Add(Grammar.Cardinal(num2) + " sets of " + item);
				}
				else
				{
					list2.Add(Grammar.Cardinal(num2) + " " + Grammar.Pluralize(item));
				}
			}
			return list2;
		}
				...
				for (int k = num2; k < num4; k++)
				{
					...
					if (bodyPart3 != null)
					{
						...
						if (bodyPart != null)
						{
							...
							bodyPart5 = body.AddPartAt(bodyPart, @base, laterality, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, DoUpdate: false);
						}
						else
						{
