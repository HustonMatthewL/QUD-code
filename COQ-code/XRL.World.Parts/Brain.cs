...
namespace XRL.World.Parts
{
	...
	public class Brain : IPart
	{
		...
		public void SetAlliedLeader<T>(GameObject Object) where T : IAllyReason, new()
		{
			TakeAllegiance<T>(Object);
			SetPartyLeader(Object);
		}

		public void SetPartyLeader(GameObject Object, int Flags = 0, bool Transient = false)
		{
		...
		public void StopFighting(GameObject Target, bool Involuntary = false)
		{
			...
			if (ParentObject.HasRegisteredEvent("StopFighting"))
			{
				Event @event = Event.New("StopFighting");
				...
				if (flag)
				{
					@event.SetFlag("GoalRemoved", State: true);
				}
		...
		public void StopFight(bool Involuntary = false)
		{
			...
			for (int count = list.Count; i < count; i++)
			{
				for (int j = 0; j < count; j++)
				{
					if (i != j)
					{
						list[i].StopFighting(list[j], Involuntary);
						list[j].StopFighting(list[i], Involuntary);
					}
				}
			}
		}
		...
		public void StopFight(GameObject Object, bool Involuntary = false, bool Reciprocal = false)
		{
			...
			{
				int l = 0;
				for (int count3 = list3.Count; l < count3; l++)
				{
					list2[k].StopFighting(list3[l], Involuntary);
				}
			}
		}
		...
		public void ThinkOfKilling(GameObject who, string because = null)
		{
			...
			stringBuilder.Append("I'm going to kill ").Append(who.an(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: false, Short: true, BaseOnly: false, IndicateHidden: false, SecondPerson: true, Reflexive: false, null));
		...
		public void Attacked(GameObject Attacker, GameObject Weapon = null, float Magnitude = 1f)
		{
			...
			else if (!IsHostileTowards(Attacker))
			{
				AddOpinion<OpinionAttack>(Attacker, Weapon, Magnitude);
				AIHelpBroadcastEvent.Send(ParentObject, Attacker, null, null, 20, 1f, HelpCause.Assault);
				if (IsHostileTowards(Attacker) && Target == null && CanFight())
				{
		...
		public override bool HandleEvent(GetShortDescriptionEvent E)
		{
			...
			{
				E.Postfix.Append("\nFighting {{r|" + Target.an(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: true, WithoutTitles: false, Short: true, BaseOnly: false, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + "}}");
			}
			return base.HandleEvent(E);
		}
		...
		public override bool HandleEvent(CommandTakeActionEvent E)
		{
			...
			if (!ParentObject.IsPlayer())
			{
				...
				if (ParentObject.HasEffect<XRL.World.Effects.Confused>())
				{
					...
					ParentObject.Move(randomDirection, Forced: false, System: false, IgnoreGravity: false, NoStack: false, AllowDashing: true, DoConfirmations: true, null, null, NearestAvailable: false, null, null, null);
					return true;
				}
				if (DoReequip)
				{
			...
			if (!num && JoinPartyLeaderPossibleEvent.Check(ParentObject, partyLeader, cell, ref TargetCell, ParentObject.IsMobile()))
			{
				...
				if (!TargetCell.ParentZone.IsWorldMap())
				{
					...
					if (cell2 != null)
					{
						...
						ParentObject.SystemLongDistanceMoveTo(cell2, null);
						ParentObject.UseEnergy(1000, "Move Join Leader", null, null);
		...
		public bool IsSuitableTarget(GameObject Object)
		{
			if (ParentObject.IsPlayerControlled())
			{
				...
				if ((Object.IsHostileTowards(The.Player) || Object.IsHostileTowards(ParentObject)) && CheckPerceptionOf(Object))
				{
					if (Object.DistanceTo(The.Player) <= Object.GetHostileWalkRadius(The.Player))
					{
						return true;
					}
					if (ParentObject != The.Player && Object.DistanceTo(ParentObject) <= Object.GetHostileWalkRadius(ParentObject))
					{
						return true;
					}
					GameObject target = Object.Target;
					if (target != null && target != The.Player && target != ParentObject && target.IsPlayerLed() && Object.DistanceTo(target) <= Object.GetHostileWalkRadius(target))
					{
						return true;
					}
				}
		...
		public override bool HandleEvent(AIHelpBroadcastEvent E)
		{
			...
			if (E.Cause == HelpCause.Assault)
			{
				if (feeling >= 50 && feeling >= GetFeeling(E.Target))
				{
					...
					{
						WantToKill(E.Target, "to help " + E.Actor.an(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: false, Short: true, BaseOnly: true, IndicateHidden: false, SecondPerson: true, Reflexive: false, null));
						return true;
					}
				}
			}
			else if (E.Cause == HelpCause.Murder || E.Cause == HelpCause.Killed)
			{
				if (feeling >= 50 && ((E.Cause == HelpCause.Murder && feeling >= GetFeeling(E.Target)) || InSamePartyAs(E.Actor) || InSameFactionAs(E.Actor)))
				{
					...
					if (flag && IsHostileTowards(E.Target))
					{
						WantToKill(E.Target, "to avenge " + E.Actor.an(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: false, Short: true, BaseOnly: true, IndicateHidden: false, SecondPerson: true, Reflexive: false, null));
						return true;
					}
				}
			}
			else
			{
				foreach (string faction in E.Factions)
				{
					...
					if (flag && IsHostileTowards(E.Target))
					{
						WantToKill(E.Target, (E.Actor != null) ? ("to protect " + E.Actor.an(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: false, Short: true, BaseOnly: true, IndicateHidden: false, SecondPerson: true, Reflexive: false, null)) : ("for " + Faction.GetFormattedName(faction)));
						return true;
					}
				}
			}
		...
		public static double PreciseWeaponScore(GameObject obj, GameObject who = null)
		{
			...
			if (obj.HasRegisteredEvent("AdjustWeaponScore"))
			{
				...
				Event @event = Event.New("AdjustWeaponScore", "Score", num7, "OriginalScore", num5, "User", who);
				obj.FireEvent(@event);
		...
		public static double PreciseMissileWeaponScore(GameObject obj, GameObject who = null)
		{
			...
			GetMissileWeaponPerformanceEvent @for = GetMissileWeaponPerformanceEvent.GetFor(who, obj, null, null, null, null, null, null, null, null, null);
			if (@for.BaseDamage.IsNullOrEmpty())
			{
				return 0.0;
			}
			...
			try
			{
				...
				if (body != null && inventory != null)
				{
					...
					foreach (BodyPart item12 in parts)
					{
						...
						GameObject gameObject4 = item12.Equipped ?? item12.DefaultBehavior;
						...
						foreach (GameObject item13 in list7)
						{
							...
							if (item12.Type == "Hand")
							{
								...
								if (!ParentObject.IsPlayer() && IsNewGearBetter(item13, gameObject4))
								{
									@object = item13;
									break;
								}
							...
							{
								if (item13.Physics != null && item13.Physics.Category == "Grenades" && IsNewGearBetter(item13, gameObject4))
								{
									@object = item13;
									break;
								}
								if (TreatAsThrownWeapon(item13) && !item13.HasTagOrProperty("NoAIEquip") && (list3 == null || !list3.Contains(item13.Blueprint)) && IsNewGearBetter(item13, gameObject4))
								{
									@object = item13;
									break;
								}
							...
							if (part != null)
							{
								if ((!part.FiresManually || part.ValidSlotType(item12.Type)) && IsNewMissileWeaponBetter(item13, gameObject4))
								{
									@object = item13;
									break;
								}
							}
							else if (item12.Type != "Missile Weapon" && IsNewGearBetter(item13, gameObject4))
							{
								@object = item13;
								break;
							}
						}
						Equip(@object, item12, list6, secondaryActiveList, tertiaryActiveList, ref LightSources, ref Shield, Silent, Initial);
					}
				}
