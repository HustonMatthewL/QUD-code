...
namespace XRL.World.Parts
{
	...
	public class TemperatureController : IPoweredPart
	{
		...
		{
			if (IsObjectActivePartSubject(E.Object) && IsReady(UseCharge: true, IgnoreCharge: false, IgnoreLiquid: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, Math.Max(Math.Abs(E.Amount) * TemperatureAmountChargeUsePercentage / 100, 1), UseChargeIfUnpowered: false, 0L, null))
			{
				return false;
			}
			return base.HandleEvent(E);
		...
		{
			if (WorksOnAdjacentCellContents && TemperatureAmountFloor > 0 && IsReady(UseCharge: false, IgnoreCharge: false, IgnoreLiquid: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L, null))
			{
				return false;
			}
			return base.HandleEvent(E);
		...
		{
			if (WorksOnCellContents && TemperatureAmountFloor > 0 && IsReady(UseCharge: false, IgnoreCharge: false, IgnoreLiquid: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L, null))
			{
				return false;
			}
			return base.HandleEvent(E);
		...
		public bool AdjustTemperature(GameObject obj, int PowerLoad = int.MinValue)
		{
			...
			obj.TemperatureChange(num2, obj.Equipped ?? obj.Implantee ?? obj, Radiant: false, MinAmbient: false, MaxAmbient: false, IgnoreResistance, 0, null, null);
			CheckMessaging(obj);
			return true;
		}
		...
		public void ConfigureTemperatureController(GameObject Actor, bool FreeAction = false)
		{
			...
			if (Actor.IsPlayer())
			{
				...
				if (num.HasValue && num != TemperatureTarget)
				{
					...
					{
						Actor.UseEnergy(ConfigureEnergy, "Item Temperature Controller", null, null);
					}
				}
			...
			else
			{
				...
				int @for = GetIdealTemperatureEvent.GetFor(Actor);
				@for = Math.Max(@for, MIN_TEMPERATURE);
				@for = Math.Min(@for, MAX_TEMPERATURE);
				if (@for != TemperatureTarget)
				{
					TemperatureTarget = @for;
					if (ConfigureEnergy > 0 && !FreeAction)
					{
