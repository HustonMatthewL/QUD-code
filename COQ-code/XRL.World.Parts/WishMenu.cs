...
namespace XRL.World.Parts
{
	...
	public class WishMenu : IPlayerPart
	{
			...
			try
			{
				...
				else
				{
					...
					if (GameObjectFactory.Factory.Blueprints.ContainsKey(text))
					{
						...
						wishCommandXML.DisplayName = "Spawn " + result.an(int.MaxValue, null, null, AsIfKnown: true, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: false, Short: true, BaseOnly: false, IndicateHidden: false, SecondPerson: true, Reflexive: false, null);
		...
		{
			CacheInit();
			int num = Popup.ShowOptionList("Wish Menu", MenuItems.Select((WishCommandXML a) => a.DisplayText).ToArray(), null, 0, null, 60, RespectOptionNewlines: false, AllowEscape: true, 0, "", null, null, MenuItems.Select((WishCommandXML a) => a.Renderable).ToArray());
			if (num < 0)
			{
				return;
			}
