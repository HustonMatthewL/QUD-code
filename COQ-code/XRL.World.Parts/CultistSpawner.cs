	...
	public class CultistSpawner : IPart
	{
		...
		public override bool FireEvent(Event E)
		{
			if (E.ID == "EnteredCell" && CultistType == "Tomb Cultist")
			{
				...
				HistoricEntitySnapshot currentSnapshot2 = XRLCore.Core.Game.sultanHistory.GetEntitiesByDelegate(delegate(HistoricEntity e)
				{
					HistoricEntitySnapshot currentSnapshot = e.GetCurrentSnapshot();
					return currentSnapshot.GetProperty("type") == "sultan" && int.Parse(currentSnapshot.GetProperty("period")) == Period;
				...
				GameObject gameObject = GetLikedFactionMember(currentSnapshot2, (GameObjectBlueprint c) => c.Tier == Tier && !c.HasTag("Merchant"));
				if (gameObject == null)
				{
				...
				TombCultistTemplate.Apply(gameObject, currentSnapshot2);
