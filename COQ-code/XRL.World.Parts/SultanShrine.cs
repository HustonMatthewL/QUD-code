...
namespace XRL.World.Parts
{
	...
	public class SultanShrine : IPart
	{
		...
		public static string GetStatueForSultan(string sultanName, int period)
		{
			...
			{
				text = "Terrain/sw_resheph_sultanstatue.bmp";
			}
			else
			{
				...
				if (dictionary.ContainsKey(sultanName))
				{
					text = dictionary[sultanName];
				}
				else
				{
					...
					for (int i = 1; i <= 11; i++)
					{
						if (!list.Contains("Terrain/sw_sultanstatue_" + i + ".png"))
						{
							for (int j = 0; j < 8; j++)
							{
								list2.Add("Terrain/sw_sultanstatue_" + i + ".png");
							}
						}
						if (i <= 8 && !list.Contains("Terrain/sw_sultanstatue_rare_" + i + ".bmp"))
						{
							list2.Add("Terrain/sw_sultanstatue_rare_" + i + ".bmp");
						}
					}
					text = list2.GetRandomElement();
					dictionary.Add(sultanName, text);
				}
			}
			return text;
		}
