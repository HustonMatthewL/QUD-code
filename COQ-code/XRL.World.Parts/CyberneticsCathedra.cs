...
namespace XRL.World.Parts
{
	...
	public class CyberneticsCathedra : IPart, IFlightSource
	{
			...
			else if (E.Command == FlightEvent)
			{
				if (E.Actor.IsActivatedAbilityToggledOn(FlightActivatedAbilityID))
				{
					if (E.Actor.IsPlayer() && E.Actor.CurrentCell != null && E.Actor.GetEffectCount(typeof(Flying)) <= 1)
					{
						...
						{
							if (@object.TryGetPart<StairsDown>(out var Part) && Part.IsLongFall() && Popup.WarnYesNo("It looks like a long way down " + @object.the + @object.ShortDisplayName + " you're above. Are you sure you want to stop flying?") != 0)
							{
								return false;
							}
						}
		...
		{
			Flight.CheckFlight(ParentObject, E.Actor, this);
			return base.HandleEvent(E);
		}
