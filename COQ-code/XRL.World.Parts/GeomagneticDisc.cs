	...
	public class GeomagneticDisc : IPoweredPart
	{
		...
		public override bool FireEvent(Event E)
		{
			if (E.ID == "BeforeThrown")
			{
				...
				IThrownWeaponFlexPhaseProvider @for = GetThrownWeaponFlexPhaseProviderEvent.GetFor(ParentObject, gameObjectParameter);
				@for?.ThrownWeaponFlexPhaseStart(ParentObject);
				try
				{
					...
					if (gameObject == null || gameObject == gameObjectParameter)
					{
						...
						if (list.Count > 0)
						{
							...
							foreach (GameObject item in list)
							{
								...
								{
									FindPath findPath2 = new FindPath(gameObjectParameter.CurrentCell, item.CurrentCell, PathGlobal: false, PathUnlimited: true, ParentObject, 99, ExploredOnly: false, Juggernaut: false, IgnoreCreatures: true, IgnoreGases: true, @for?.ThrownWeaponFlexPhaseIsActive(ParentObject) ?? false);
					...
					if (findPath == null)
					{
						findPath = new FindPath(gameObjectParameter.CurrentCell, gameObject.CurrentCell, PathGlobal: false, PathUnlimited: true, ParentObject, 99, ExploredOnly: false, Juggernaut: false, IgnoreCreatures: true, IgnoreGases: true, @for?.ThrownWeaponFlexPhaseIsActive(ParentObject) ?? false);
					...
					gameObjectParameter.UseEnergy(1000, "Physical Skill Throwing Item", null, null);
					if (firstThrownWeapon != null)
					{
						IComponent<GameObject>.EmitMessage(gameObjectParameter, ParentObject.T(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + " and " + firstThrownWeapon.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + " collide and fall to the ground.");
					...
					for (int i = 0; i < num4 && i < list2.Count; i++)
					{
						List<GameObject> list4 = list2[i].CurrentCell.FastFloodSearch("Brain", num3);
						if (list4.Count <= 0)
						{
							continue;
						}
						...
						foreach (GameObject item2 in list4)
						{
							...
							FindPath findPath3 = new FindPath(list2[i].CurrentCell, item2.CurrentCell, PathGlobal: false, PathUnlimited: true, ParentObject, 99, ExploredOnly: false, Juggernaut: false, IgnoreCreatures: true, IgnoreGases: true, @for?.ThrownWeaponFlexPhaseIsActive(ParentObject) ?? false);
					...
					DoThrow(gameObjectParameter, list3, UnidentifiedMessage: false, UsePopups: false, list2, null, E.GetIntParameter("Phase"), num2, @for, E);
					return false;
				}
				finally
				{
					@for?.ThrownWeaponFlexPhaseEnd(ParentObject);
				}
			}
		...
		private void DoThrow(GameObject Actor, List<FindPath> Paths, bool UnidentifiedMessage = false, bool UsePopups = false, List<GameObject> Targets = null, GameObject LastTarget = null, int Phase = 0, int? Bonus = null, IThrownWeaponFlexPhaseProvider FlexPhaseProvider = null, IEvent ParentEvent = null)
		{
			...
			try
			{
				...
				RenderEvent r = ((flag && !flag2) ? ParentObject.RenderForUI(AsIfKnown: true) : null);
				int i = 0;
				for (int count = Paths.Count; i < count; i++)
				{
					...
					int j = 0;
					for (int num3 = findPath2.Steps.Count - 1; j <= num3; j++)
					{
						findPath2.Steps[j].WakeCreaturesInArea();
						if (FlexPhaseProvider != null)
						{
							if (!FlexPhaseProvider.ThrownWeaponFlexPhaseTraversal(Actor, (gameObject != null && findPath2.Steps[j].Objects.Contains(gameObject)) ? gameObject : null, gameObject, ParentObject, Phase, cell, findPath2.Steps[j], out var _, out var RecheckPhase, HasDynamicTargets: true))
							{
								return;
							}
							if (RecheckPhase)
						...
						if (RelationManager != null)
						{
							findPath2.Steps[j].AddObject(ParentObject, Forced: false, System: false, IgnoreGravity: true, NoStack: true, Silent: false, Repaint: true, FlushTransient: true, null, "Thrown", null, Actor, null, null, null, ParentEvent);
							if (!GameObject.Validate(ParentObject) || ParentObject.CurrentCell != findPath2.Steps[j])
							{
								return;
							}
						}
						...
						{
							bool flag3 = false;
							projectileMovingEvent.Cell = findPath2.Steps[j];
							projectileMovingEvent.PathIndex = j;
							...
							if (flag3)
							{
								...
								if (findPath2 != findPath)
								{
									findPath = new FindPath(findPath2.Steps[j], Paths[0].Steps[0], PathGlobal: false, PathUnlimited: true, ParentObject, 99, ExploredOnly: false, Juggernaut: false, IgnoreCreatures: true, IgnoreGases: true, FlexPhaseProvider?.ThrownWeaponFlexPhaseIsActive(ParentObject) ?? false);
									if (findPath.Usable)
									{
						...
						if (flag && !flag2)
						{
							if (j == num3 && findPath2 != findPath)
							{
								scrapBuffer.Copy(scrapBuffer2);
								scrapBuffer.Goto(findPath2.Steps[j].X, findPath2.Steps[j].Y);
					...
					if (num5 > combatDV || num4 == 20)
					{
						if (gameObject.PhaseMatches(Phase))
						{
							...
							@event.SetParameter("Defender", gameObject);
							@event.SetParameter("Weapon", ParentObject);
							@event.SetParameter("Projectile", ParentObject);
							@event.SetParameter("ApparentTarget", gameObject);
							ParentObject.FireEvent(@event);
							value = @event.GetIntParameter("Damage");
		...
		private void SignalLowPower(GameObject actor)
		{
			if (actor != null && actor.IsPlayer())
			{
				Popup.ShowFail("A loud buzz is emitted. The low power glyph flashes on the side of " + ParentObject.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + ".");
			}
		}
				...
				if (IsReady(UseCharge: true, IgnoreCharge: false, IgnoreLiquid: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: true, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L, powerLoadLevel))
				{
					...
					if (cell != null)
					{
						...
						if (list2.Count > 0)
						{
							if (E.Actor.IsPlayer())
							{
								Popup.Show(ParentObject.T(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + " suddenly" + ParentObject.GetVerb("start") + " flying around!");
							}
