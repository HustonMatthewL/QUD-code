...
namespace XRL.World.Parts
{
	...
	public class MutationOnEquip : IActivePart
	{
		...
		public override void Initialize()
		{
			if (!ClassName.IsNullOrEmpty())
			{
				...
				if (genericMutation != null)
				{
					if (DisplayName == null)
					{
						DisplayName = genericMutation.DisplayName;
					}
		...
		public void CheckApplyBonus(GameObject Subject, bool UseCharge = false)
		{
			if (MutationWasAdded)
			{
				if (!IsReady(UseCharge, IgnoreCharge: false, IgnoreLiquid: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L, null) || !IsObjectActivePartSubject(Subject))
				{
					UnapplyBonus(Subject);
				}
			}
			else if (IsReady(UseCharge, IgnoreCharge: false, IgnoreLiquid: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L, null))
			{
				ApplyBonus(Subject);
			}
		}
