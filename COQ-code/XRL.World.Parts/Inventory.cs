...
namespace XRL.World.Parts
{
	...
	public class Inventory : IPart, IInventory
	{
		...
		public GameObject AddObject(string Blueprint, bool Silent = false, bool NoStack = false)
		{
			...
			return AddObject(@object, null, Silent, noStack);
		}
		...
		public GameObject AddObject(GameObject Object, GameObject Actor = null, bool Silent = false, bool NoStack = false, bool FlushTransient = true, IEvent ParentEvent = null)
		{
			...
			if (physics == null || !physics.Takeable)
			{
				...
				{
					AutoAct.Interrupt(Object.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + " can't be picked up");
				}
				return Object;
			}
		...
		public override bool FireEvent(Event E)
		{
			...
			else if (E.ID == "PerformEquip")
			{
				...
				if (Object != null)
				{
					Event event2 = Event.New("CommandUnequipObject", "BodyPart", bodyPart, "Sound", "");
					if (flag3)
					{
					...
					{
						event2.SetParameter("FailureMessage", stringParameter);
					}
					if (flag4)
					{
						event2.SetFlag("DestroyOnUnequipDeclined", State: true);
					}
					if (!ParentObject.FireEvent(event2))
					{
						...
						if (event2.HasFlag("DestroyOnUnequipDeclined"))
						{
							flag4 = true;
				...
				if (!flag3)
				{
					...
					if (ParentObject.IsPlayer())
					{
						...
						if (!Value.IsNullOrEmpty())
						{
							DelimitedStringEnumerator enumerator = Value.DelimitedBy(',').GetEnumerator();
			...
			else if (E.ID == "CommandRemoveObject")
			{
				...
				if (gameObjectParameter5 != null)
				{
					if (!ParentObject.HasRegisteredEvent("BeginDrop") || ParentObject.FireEvent(Event.New("BeginDrop", "Object", gameObjectParameter5, "ForEquip", E.GetIntParameter("ForEquip"))))
					{
						...
						{
							Event event3 = Event.New("PerformDrop");
							event3.SetParameter("Object", gameObjectParameter5);
							if (E.HasFlag("Forced"))
							{
								event3.SetFlag("Forced", State: true);
							}
							if (E.IsSilent())
							{
								event3.SetSilent(Silent: true);
							}
							return ParentObject.FireEvent(event3);
						}
						return false;
					}
					return false;
			...
			else if (E.ID == "CommandEquipObject" || E.ID == "CommandForceEquipObject")
			{
				...
				try
				{
					...
					List<BodyPart> @for = QuerySlotListEvent.GetFor(ParentObject, Object2, ref FailureMessage2);
					if (!flag6 && !flag7)
					{
						if (@for.Count == 0)
						{
							if (!flag8 && ParentObject.IsPlayer())
							{
								...
								{
									FailureMessage2 = "You cannot equip " + Object2.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + ".";
								}
								if (intParameter2 > 0)
								{
									E.SetParameter("FailureMessage", FailureMessage2);
						...
						if (bodyPart3 != null && !@for.Contains(bodyPart3))
						{
							if (!flag8 && ParentObject.IsPlayer())
							{
								if (FailureMessage2.IsNullOrEmpty())
								{
									FailureMessage2 = "You cannot equip " + Object2.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + " on your " + bodyPart3.GetOrdinalName() + ".";
								}
								if (intParameter2 > 0)
								{
									E.SetParameter("FailureMessage", FailureMessage2);
					...
					{
						List<string> list2 = new List<string>(@for.Count);
						List<char> list3 = new List<char>(@for.Count);
						char c = 'a';
						...
						{
							if (@for.Any((BodyPart p) => p.Primary))
							{
								defaultSelected = @for.IndexOf(@for.First((BodyPart p) => p.Primary));
							}
						}
						else if (@for.Any((BodyPart p) => p.Type != "Hand"))
						{
							defaultSelected = @for.IndexOf(@for.First((BodyPart p) => p.Type != "Hand"));
						}
						int num2 = Popup.PickOption("", null, "", "Sounds/UI/ui_notification", list2.ToArray(), list3.ToArray(), null, null, null, null, null, 0, 60, defaultSelected, -1, AllowEscape: true);
						if (num2 == -1)
						{
							return false;
						}
					...
					num = E.GetIntParameter("EnergyCost", @default);
					...
					{
						Event event4 = Event.New("BeginEquip", "Object", Object2, "BodyPart", bodyPart3);
						if (flag8)
						{
						...
						if (intParameter2 > 0)
						{
							event4.SetParameter("AutoEquipTry", intParameter2);
						}
						if (!FailureMessage2.IsNullOrEmpty())
						{
							event4.SetParameter("FailureMessage", FailureMessage2);
						}
						if (list != null)
						{
						...
						if (flag9)
						{
							event4.SetFlag("DestroyOnUnequipDeclined", State: true);
						}
						bool num3 = ParentObject.FireEvent(event4);
						string stringParameter4 = event4.GetStringParameter("FailureMessage");
						if (!stringParameter4.IsNullOrEmpty() && stringParameter4 != FailureMessage2)
						{
							FailureMessage2 = stringParameter4;
							E.SetParameter("FailureMessage", FailureMessage2);
						}
						if (event4.HasFlag("DestroyOnUnequipDeclined"))
						{
							flag9 = true;
							E.SetFlag("DestroyOnUnequipDeclined", State: true);
						}
					...
					{
						Event event5 = Event.New("BeginBeingEquipped", "EquippingObject", ParentObject, "Equipper", ParentObject, "BodyPart", bodyPart3);
						if (flag8)
						{
						...
						if (intParameter2 > 0)
						{
							event5.SetParameter("AutoEquipTry", intParameter2);
						}
						if (!FailureMessage2.IsNullOrEmpty())
						{
							event5.SetParameter("FailureMessage", FailureMessage2);
						}
						if (list != null)
						{
						...
						if (flag9)
						{
							event5.SetFlag("DestroyOnUnequipDeclined", flag9);
						}
						bool num4 = Object2.FireEvent(event5);
						string stringParameter5 = event5.GetStringParameter("FailureMessage");
						if (!stringParameter5.IsNullOrEmpty() && stringParameter5 != FailureMessage2)
						{
							FailureMessage2 = stringParameter5;
							E.SetParameter("FailureMessage", FailureMessage2);
						}
						if (event5.HasFlag("DestroyOnUnequipDeclined"))
						{
							flag9 = true;
							E.SetFlag("DestroyOnUnequipDeclined", State: true);
						}
					...
					Event event6 = Event.New("PerformEquip", "Object", Object2);
					event6.SetParameter("BodyPart", bodyPart3);
					if (E.TryGetStringParameter("Sound", out var Value3))
					{
						event6.SetParameter("Sound", Value3);
					}
					if (flag8)
					{
					...
					if (intParameter2 > 0)
					{
						event6.SetParameter("AutoEquipTry", intParameter2);
					}
					if (!FailureMessage2.IsNullOrEmpty())
					{
						event6.SetParameter("FailureMessage", FailureMessage2);
					}
					if (list != null)
					{
					...
					if (flag9)
					{
						event6.SetFlag("DestroyOnUnequipDeclined", State: true);
					}
					bool num5 = ParentObject.FireEvent(event6);
					string stringParameter6 = event6.GetStringParameter("FailureMessage");
					if (!stringParameter6.IsNullOrEmpty() && stringParameter6 != FailureMessage2)
					{
						FailureMessage2 = stringParameter6;
						E.SetParameter("FailureMessage", FailureMessage2);
					}
					if (event6.HasFlag("DestroyOnUnequipDeclined"))
					{
						flag9 = true;
						E.SetFlag("DestroyOnUnequipDeclined", State: true);
					}
					if (!num5 && !flag6)
					{
						Event event7 = Event.New("CommandTakeObject");
						event7.SetParameter("Object", Object2);
						event7.SetParameter("Context", E.GetStringParameter("Context"));
						event7.SetSilent(Silent: true);
						if (!ParentObject.FireEvent(event7))
						{
							if (ParentObject.CurrentCell != null)
							{
				...
				if (flag10 && num > 0)
				{
					ParentObject.UseEnergy(num, "Equip Item", null, null);
				}
			}
			else if (E.ID == "CommandUnequipObject" || E.ID == "CommandForceUnequipObject")
			{
				...
				Event event8 = Event.New("BeginUnequip", "BodyPart", bodyPart4);
				event8.SetFlag("Forced", flag11);
				event8.SetFlag("SemiForced", flag12);
				if (flag13)
				{
					event8.SetFlag("NoStack", State: true);
				}
				if (flag14)
				{
				...
				if (intParameter3 > 0)
				{
					event8.SetParameter("AutoEquipTry", intParameter3);
				}
				if (!FailureMessage3.IsNullOrEmpty())
				{
					event8.SetParameter("FailureMessage", FailureMessage3);
				}
				if (DestroyOnUnequipDeclined)
				{
					event8.SetFlag("DestroyOnUnequipDeclined", DestroyOnUnequipDeclined);
				}
				if (!ParentObject.FireEvent(event8))
				{
					string stringParameter7 = event8.GetStringParameter("FailureMessage");
					...
					if (event8.HasFlag("DestroyOnUnequipDeclined"))
					{
				...
				bool num6 = Object3.BeginBeingUnequipped(ref FailureMessage3, ref DestroyOnUnequipDeclined, ParentObject, ParentObject, bodyPart4, flag14, flag11, flag12, intParameter3);
				E.SetParameter("FailureMessage", FailureMessage3);
				...
				Event event9 = Event.New("PerformUnequip");
				event9.SetParameter("BodyPart", bodyPart4);
				event9.SetFlag("Forced", flag11);
				event9.SetFlag("SemiForced", flag12);
				if (E.TryGetStringParameter("Sound", out var Value4))
				{
					event9.SetParameter("Sound", Value4);
				}
				if (flag13)
				{
					event9.SetFlag("NoStack", State: true);
				}
				if (flag14)
				{
				...
				if (intParameter3 > 0)
				{
					event9.SetParameter("AutoEquipTry", intParameter3);
				}
				if (!FailureMessage3.IsNullOrEmpty())
				{
					event9.SetParameter("FailureMessage", FailureMessage3);
				}
				if (DestroyOnUnequipDeclined)
				{
					event9.SetFlag("DestroyOnUnequipDeclined", State: true);
				}
				if (!ParentObject.FireEvent(event9))
				{
					string stringParameter8 = event9.GetStringParameter("FailureMessage");
					...
					if (event9.HasFlag("DestroyOnUnequipDeclined"))
					{
						DestroyOnUnequipDeclined = true;
						E.SetFlag("DestroyOnUnequipDeclined", State: true);
					}
			...
			else if (E.ID == "CommandTakeObject")
			{
				...
				{
					return ParentObject.Fail("You cannot budge " + gameObjectParameter6.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + ".");
				}
				...
				if (inInventory2 != null)
				{
					...
					if (inInventory2.IsOwned() && ParentObject.IsPlayer() && inInventory2.HasTagOrProperty("DontWarnOnOpen") && gameObjectParameter6.GetIntProperty("StoredByPlayer") <= 0 && gameObjectParameter6.GetIntProperty("FromStoredByPlayer") <= 0 && Popup.ShowYesNo("You don't own " + inInventory2.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, inInventory2.indicativeProximal, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + ". Are you sure you want to take " + gameObjectParameter6.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + "?") != 0)
					{
						return false;
					}
				...
				Event event10 = Event.New("BeginTake");
				event10.SetParameter("Object", gameObjectParameter6);
				event10.SetParameter("PutBy", gameObjectParameter7);
				event10.SetParameter("Container", inInventory2);
				event10.SetParameter("Context", stringParameter9);
				event10.SetSilent(E.IsSilent());
				if (flag15)
				{
					event10.SetFlag("NoStack", State: true);
				}
				if (!ParentObject.FireEvent(event10, E))
				{
					return false;
				}
				Event event11 = Event.New("BeginBeingTaken");
				event11.SetParameter("TakingObject", ParentObject);
				event11.SetParameter("PutBy", gameObjectParameter7);
				event11.SetParameter("Container", inInventory2);
				event11.SetParameter("Context", stringParameter9);
				event11.SetSilent(E.IsSilent());
				if (flag15)
				{
					event11.SetFlag("NoStack", State: true);
				}
				if (!gameObjectParameter6.FireEvent(event11, E))
				{
					return false;
				}
				Event event12 = Event.New("PerformTake");
				event12.SetParameter("Object", gameObjectParameter6);
				event12.SetParameter("PutBy", gameObjectParameter7);
				event12.SetParameter("Container", inInventory2);
				event12.SetParameter("Context", stringParameter9);
				event12.SetSilent(E.IsSilent());
				if (flag15)
				{
					event12.SetFlag("NoStack", State: true);
				}
				if (!ParentObject.FireEvent(event12, E))
				{
					return false;
				}
				if (intParameter4 > 0)
				{
					ParentObject.UseEnergy(intParameter4, "Take", stringParameter9, null);
				}
			}
		...
		public override bool HandleEvent(InventoryActionEvent E)
		{
			if (E.Command == "CommandDropObject" || E.Command == "CommandDropAllObject")
			{
				...
				Event @event = Event.New("PerformDrop");
				@event.SetParameter("Object", E.Item);
				if (E.Forced)
				{
				...
				if (!E.Actor.FireEvent(@event) && !E.Forced)
				{
					return false;
				}
				...
				if (dropInventory != null)
				{
					...
					for (int count = list.Count; i < count; i++)
					{
						...
						if (gameObject.IsReal && DropOnDeathEvent.Check(gameObject, dropInventory))
						{
							...
							gameObject.HandleEvent(DroppedEvent.FromPool(ParentObject, gameObject));
						}
					}
				}
				else
