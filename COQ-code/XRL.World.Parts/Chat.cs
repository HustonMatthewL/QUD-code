		...
		public void PerformChat(GameObject who, bool Dialog)
		{
			...
			else if (text[0] == '@')
			{
				...
				foreach (string text2 in array)
				{
					...
					{
						IComponent<GameObject>.EmitMessage(gameObject, ParentObject.Does("say", int.MaxValue, null, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, WithIndefiniteArticle: false, null, IndicateHidden: false, Pronoun: false, SecondPerson: true, null) + ", '{{|" + text2 + "}}'.", ' ', Dialog);
					}
				}
			}
			else
			{
				...
				if (!text3.IsNullOrEmpty())
				{
					...
					{
						IComponent<GameObject>.EmitMessage(gameObject, ParentObject.Does("say", int.MaxValue, null, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, WithIndefiniteArticle: false, null, IndicateHidden: false, Pronoun: false, SecondPerson: true, null) + ", '{{|" + text3 + "}}'.", ' ', Dialog);
					}
				}
			}
			if (who.IsPlayer())
