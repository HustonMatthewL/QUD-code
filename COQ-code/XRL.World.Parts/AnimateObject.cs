				...
				if (cell != null)
				{
					...
					else
					{
						...
						if (num >= 0)
						{
							...
							else if (IsDisabled(UseCharge: true, IgnoreCharge: false, IgnoreLiquid: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L, null))
							{
								if (E.Actor.IsPlayer())
								{
									Popup.Show(ParentObject.Does("are", int.MaxValue, null, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, WithIndefiniteArticle: false, null, IndicateHidden: false, Pronoun: false, SecondPerson: true, null) + " unresponsive.");
								}
							}
							else
							{
								gameObject.PlayWorldSound("Sounds/Interact/sfx_interact_sentience_imbue");
								E.Actor.UseEnergy(1000, "Item Animate", null, null);
								if (E.Actor.IsPlayer())
								{
									Popup.Show("You imbue " + gameObject.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + " with life.");
									JournalAPI.AddAccomplishment("You imbued " + gameObject.an(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: false, Short: true, BaseOnly: false, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + " with life. Why?", "While traveling in " + Grammar.GetProsaicZoneName(The.Player.CurrentZone) + ", =name= performed a sacred ritual with " + gameObject.a + gameObject.ShortDisplayName + ", imbuing " + gameObject.them + " with life and arranging " + gameObject.them + " " + HistoricStringExpander.ExpandString("<spice.elements." + IComponent<GameObject>.ThePlayerMythDomain + ".babeTrait.!random>") + ". Many of the local denizens declared it a miracle. Some weren't so sure.", "While traveling in " + Grammar.GetProsaicZoneName(The.Player.CurrentZone) + ", =name= performed a sacred ritual with " + gameObject.an(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: false, Short: true, BaseOnly: false, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + ", imbuing " + gameObject.them + " with life and arranging " + gameObject.them + " " + HistoricStringExpander.ExpandString("<spice.elements." + The.Player.GetMythicDomain() + ".babeTrait.!random>") + ". Many of the local denizens declared it a miracle.", null, "general", MuralCategory.CommitsFolly, MuralWeight.Medium, null, -1L);
								}
