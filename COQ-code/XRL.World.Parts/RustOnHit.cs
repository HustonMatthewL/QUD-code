		...
		public override bool FireEvent(Event E)
		{
			if (E.ID.EndsWith("Hit"))
			{
				...
				if (GameObject.Validate(ref Object))
				{
					...
					int @for = GetSpecialEffectChanceEvent.GetFor(gameObjectParameter, gameObjectParameter2, "Part RustOnHit Activation", Chance, subject, projectile);
					bool flag = AffectInventory;
					bool flag2 = AffectEquipment;
					if (Object.HasRegisteredEvent("GetRustableContents"))
					{
						Event @event = Event.New("GetRustableContents", "Inventory", flag ? 1 : 0, "Equipment", flag2 ? 1 : 0);
						Object.FireEvent(@event);
						flag = @event.GetIntParameter("Inventory") >= 1;
						flag2 = @event.GetIntParameter("Equipment") >= 1;
						if (!flag && !flag2)
						{
							return base.FireEvent(E);
						}
