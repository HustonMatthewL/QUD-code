...
namespace XRL.World.Parts
{
	...
	public class TerrainTravel : IPart
	{
						...
						if (num3.in100())
						{
							...
							if (encounter.secretID.IsNullOrEmpty() || !JournalAPI.GetMapNote(encounter.secretID).Revealed)
							{
								...
								if (!encounter.Optional || Popup.ShowYesNo(encounter.Text) == DialogResult.Yes)
								{
									...
									zone.CheckWeather();
									E.Object.SystemMoveTo(zone.GetPullDownLocation(E.Object), null);
		...
		{
			int @for = TravelSpeedEvent.GetFor(Object, TravelClass);
			return (int)(300f / ((float)Object.Speed * (100f + (float)@for) / 100f) * (float)Segments);
		}
			...
			if (chance.in100())
			{
				...
				if (!The.ZoneManager.IsZoneBuilt(text))
				{
					...
					if (pullDownLocation.IsPassable(The.Player))
					{
						...
						{
							zone.CheckWeather();
							The.Player.SystemMoveTo(pullDownLocation, null);
