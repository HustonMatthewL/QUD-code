		...
		public static string GenerateGossip_TwoFactions(string actor, string actee)
		{
			...
			if (factionNames.Contains(actor))
			{
				if (40.in100())
				{
					...
					actor = ((factionMembers.Count <= 0) ? ((!Faction.GetFormattedName(actor).IsNullOrEmpty()) ? Faction.GetFormattedName(actor) : actor) : GameObject.Create(factionMembers.GetRandomElement().Name).an(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: true, WithoutTitles: false, Short: true, BaseOnly: false, IndicateHidden: false, SecondPerson: true, Reflexive: false, null));
				}
				else
				{
			...
			if (factionNames.Contains(actee))
			{
				if (40.in100())
				{
					...
					actee = ((factionMembers2.Count <= 0) ? ((!Faction.GetFormattedName(actee).IsNullOrEmpty()) ? Faction.GetFormattedName(actee) : actee) : GameObject.Create(factionMembers2.GetRandomElement().Name).an(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: true, WithoutTitles: false, Short: true, BaseOnly: false, IndicateHidden: false, SecondPerson: true, Reflexive: false, null));
				}
				else
				{
