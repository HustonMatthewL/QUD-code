...
namespace XRL.World.Parts
{
	...
	public class EnergyCellSocket : IPoweredPart
	{
		...
		public bool AttemptRemoveCell(GameObject Owner, InventoryActionEvent E, bool CheckStack = true)
		{
			...
			try
			{
				Event @event = Event.New("BeforeRemoveCell", "Object", cell);
				bool num = ParentObject.FireEvent(@event);
				@event.ID = "CommandTakeObject";
				if (num && E.OverrideEnergyCost)
				{
					@event.SetParameter("EnergyCost", E.EnergyCostOverride);
				}
				if (!num || Owner.Inventory == null || !Owner.Inventory.FireEvent(@event))
				{
					Owner.Fail("You can't remove " + Cell.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + "!");
					return false;
				}
				Owner.PlayWorldOrUISound("Sounds/Missile/Special/sfx_missile_energyWeapon_cellRemoved", null);
		...
		public bool AttemptReplaceCell(GameObject Actor, InventoryActionEvent E, int MinimumCharge = 0, GameObject NewCell = null)
		{
			...
			if (NewCell == null)
			{
				...
				if (Actor.IsPlayer())
				{
					choice = Popup.PickOption("Choose a cell for " + ParentObject.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null), null, "", "Sounds/UI/ui_notification", list.ToArray(), list3.ToArray(), null, null, null, null, null, 0, 60, num, -1, AllowEscape: true);
				}
				else
				{
					Cell?.ForeachPartDescendedFrom(delegate(IEnergyCell P)
			...
			if (NewCell != null)
			{
				...
				if (Cell == null)
				{
					Actor.UseEnergy(E.OverrideEnergyCost ? E.EnergyCostOverride : 1000, "Reload Energy Cell", null, null);
					flag6 = true;
				}
				...
				if (flag6)
				{
					...
					@event.SetParameter("Object", NewCell);
		...
		public override bool FireEvent(Event E)
		{
			...
			{
				if (GameObject.Validate(ref Cell) && IsReady(UseCharge: false, IgnoreCharge: false, IgnoreLiquid: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L, null) && !Cell.FireEvent(E))
				{
					return false;
				}
			}
			else if (E.ID == "UseCharge" && GameObject.Validate(ref Cell) && IsReady(E.GetIntParameter("Charge") > 0, IgnoreCharge: false, IgnoreLiquid: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L, null) && !Cell.FireEvent(E))
			{
				return false;
			}
