			...
			if (E.ID == "AppliedLiquidCovered")
			{
				...
				if (equipped != null)
				{
					...
					if (liquidVolume.ContainsLiquid(liquidID) && liquidVolume.ContainsLiquid(stringGameState))
					{
						if (!Curable)
						{
							IComponent<GameObject>.EmitMessage(equipped, ParentObject.Does("are", int.MaxValue, null, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, WithIndefiniteArticle: false, null, IndicateHidden: false, Pronoun: false, SecondPerson: true, null) + " immune to conventional treatments.", ' ', FromDialog: false, UsePopup: true);
						}
