...
namespace XRL.World.Parts
{
	...
	public class Interesting : IPart
	{
		...
		public override bool HandleEvent(GetPointsOfInterestEvent E)
		{
			if (RequirementsMet(E.Actor))
			{
				...
				if (flag)
				{
					...
					GameObject @object = ParentObject;
					...
					E.Add(@object, DisplayName ?? ParentObject.GetReferenceDisplayName(), text, text2, Preposition, location, null, Radius, renderable, Order);
				}
			}
			return base.HandleEvent(E);
		}
		...
		public bool RequirementsMet(GameObject who)
		{
			...
			if (!string.IsNullOrEmpty(IfPartOperational) && (!(ParentObject.GetPart(IfPartOperational) is IActivePart activePart) || activePart.IsDisabled(UseCharge: false, IgnoreCharge: false, IgnoreLiquid: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L, null)))
			{
				return false;
			}
			return true;
