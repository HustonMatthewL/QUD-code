...
namespace XRL.World.Parts
{
	...
	public class MechanicalWings : IPoweredPart, IFlightSource
	{
		...
		public override void TurnTick(long TimeTick, int Amount)
		{
			...
			{
				if (IsReady(UseCharge: true, IgnoreCharge: false, IgnoreLiquid: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, Amount, null, UseChargeIfUnpowered: false, 0L, null))
				{
				...
				if (User.IsActivatedAbilityToggledOn(FlightActivatedAbilityID))
				{
					if (User.IsPlayer() && base.currentCell != null && User.GetEffectCount(typeof(Flying)) <= 1)
					{
						...
						for (int count = base.currentCell.Objects.Count; i < count; i++)
						{
							...
							if (part != null && part.IsLongFall() && Popup.WarnYesNo("It looks like a long way down " + gameObject.the + gameObject.ShortDisplayName + " you're above. Are you sure you want to stop flying?") != 0)
							{
								return false;
							}
						}
