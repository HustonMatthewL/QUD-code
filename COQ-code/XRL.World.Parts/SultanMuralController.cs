	...
	public class SultanMuralController : IPart
	{
		...
		public void initializeMurals()
		{
			...
			if (Layout.StartsWith("chronological:"))
			{
				...
				for (int k = 0; k < list3.Count && k < list.Count; k++)
				{
					List<Cell> muralCells = list[k + num];
					HistoricEvent ev = list3[k];
			...
			if (Layout.StartsWith("random:"))
			{
				...
				for (int l = 0; l < list4.Count && l < list.Count; l++)
				{
					List<Cell> muralCells2 = list[l];
					HistoricEvent ev2 = list4[l];
					if (seededRandom.Next(1, 101) <= RuinedChance)
					{
		...
		public void blankMural(List<Cell> muralCells)
		{
			...
			for (int i = 0; i < 3; i++)
			{
				...
				GameObject firstObjectWithPart2 = muralCells[i].GetFirstObjectWithPart("SultanMural");
