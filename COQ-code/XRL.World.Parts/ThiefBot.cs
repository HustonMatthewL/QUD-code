					...
					if (target != null && ParentObject.DistanceTo(target) <= 1 && (ParentObject.IsFlying || !target.IsFlying))
					{
						...
						else if (target.MakeSave("Agility", 10, ParentObject, "Agility", "Disarm"))
						{
							...
							if (gameObject != null && ParentObject.TakeObject(gameObject, NoStack: false, Silent: true, 0))
							{
								...
								{
									string msg = Event.NewStringBuilder().Append(ParentObject.Does("snag", int.MaxValue, null, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, WithIndefiniteArticle: false, null, IndicateHidden: false, Pronoun: false, SecondPerson: true, null)).Compound(target.poss(gameObject, Definite: true, null))
										.Append('!')
										.ToString();
