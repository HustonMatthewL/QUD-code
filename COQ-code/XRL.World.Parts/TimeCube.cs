...
namespace XRL.World.Parts
{
	...
	public class TimeCube : IPart
	{
		...
		{
			if (!IComponent<GameObject>.CheckRealityDistortionUsability(Actor, null, Actor, ParentObject, null, null) || !IComponent<GameObject>.CheckRealityDistortionUsability(ParentObject, null, Actor, ParentObject, null, null))
			{
				if (!SilentIfFailed)
				{
					if (Actor.IsPlayer())
					...
					Actor.UseEnergy(1000, "Item Failure", null, null);
				}
				return false;
			}
			Actor.PlayWorldOrUISound("Sounds/Interact/sfx_interact_timeCube_activate", null);
			if (Actor.IsPlayer())
			{
