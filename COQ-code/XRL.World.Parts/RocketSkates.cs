...
namespace XRL.World.Parts
{
	...
	public class RocketSkates : IActivePart
	{
		...
		public override bool HandleEvent(JumpedEvent E)
		{
			...
			{
				ConsumeCharge((E.Path?.Count ?? E.OriginCell?.DistanceTo(E.TargetCell) ?? 5) * 2, null, null);
				if (E.Path != null)
				{
					bool flag = false;
		...
		{
			if (IsSkating() && IsReady(UseCharge: true, IgnoreCharge: false, IgnoreLiquid: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L, null))
			{
				ParentObject.Smoke();
			}
			SyncAbility();
		...
		public override bool FireEvent(Event E)
		{
			if (E.ID == "AfterMoved")
			{
				...
				if (cell != null && cell.ParentZone == cell2.ParentZone && PlumeLevel > 0 && !cell2.OnWorldMap() && IsSkating() && IsReady(UseCharge: false, IgnoreCharge: false, IgnoreLiquid: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L, null))
				{
		...
		private void SyncAbility(GameObject who = null)
		{
			...
			if (IsObjectActivePartSubject(who) && IsReady(UseCharge: false, IgnoreCharge: false, IgnoreLiquid: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L, null))
			{
				who.RequirePart<Run>();
			}
			Run.SyncAbility(who);
		...
		{
			if (E.Pass == 1 && GlobalConfig.GetBoolSetting("ContextualExamineFailures") && Chance.in100() && IsReady(UseCharge: true, IgnoreCharge: false, IgnoreLiquid: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: true, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L, null) && EmitFlamePlume(E.Actor.CurrentCell, ParentObject.CurrentCell, ParentObject, ShowMessage: true, E.Actor.IsPlayer()))
			{
				E.Identify = true;
				return true;
			}
