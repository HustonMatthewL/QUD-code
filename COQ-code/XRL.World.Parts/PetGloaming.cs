		...
		public override bool FireEvent(Event E)
		{
			if (E.ID == "ApplyRealityStabilized")
			{
				if (ParentObject.CurrentCell != null && gloamingBlueprint != null && Gloaming > 0)
				{
					...
					IComponent<GameObject>.AddPlayerMessage(ParentObject.Poss("astral tether") + " snaps and " + ParentObject.its + " binal specter substantiates as " + gameObject.an(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: false, Short: true, BaseOnly: false, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + ".");
			...
			else if (E.ID == "EnteredCell" && ParentObject.CurrentZone != null && !visitedZones.Contains(ParentObject.CurrentZone.ZoneID))
			{
				...
				if (SecretChance.in100())
				{
					...
					Popup.Show(ParentObject.Does("beat", int.MaxValue, null, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, WithIndefiniteArticle: false, null, IndicateHidden: false, Pronoun: false, SecondPerson: true, null) + " " + ParentObject.its + " wings, and the shattered voices of a trillion worlds ride the current of air and harmonize into one, revealing the following wisdom:\n\n" + text);
				...
				if (Gloaming > 0 && StopGloamingChance.in100())
				{
					...
					IComponent<GameObject>.AddPlayerMessage(ParentObject.Does("stop", int.MaxValue, null, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, WithIndefiniteArticle: false, null, IndicateHidden: false, Pronoun: false, SecondPerson: true, null) + " gleaming.");
				}
				else if (GloamingChance.in100())
				{
					...
					if (!ParentObject.HasEffect<Gleaming>())
					{
						...
						IComponent<GameObject>.AddPlayerMessage(ParentObject.Does("start", int.MaxValue, null, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, WithIndefiniteArticle: false, null, IndicateHidden: false, Pronoun: false, SecondPerson: true, null) + " to gleam with an {{K|unearthly light}}.");
					}
				}
			}
