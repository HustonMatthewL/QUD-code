...
namespace XRL.World.Parts
{
	...
	public class ElectricalDischargeLoader : IPoweredPart
	{
		...
		{
			if (GetActivePartStatus(UseCharge: false, IgnoreCharge: false, IgnoreLiquid: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L, null) == ActivePartStatus.Unpowered && E.Object.TryGetPart<EnergyCellSocket>(out var socket))
			{
				bool result = false;
		...
		public override bool HandleEvent(LoadAmmoEvent E)
		{
			...
			if (activePartStatus == ActivePartStatus.Operational)
			{
				...
				{
					int? powerLoadLevel2 = value;
					int activeChargeUse = GetActiveChargeUse(null, powerLoadLevel2);
		...
		public override bool HandleEvent(GetMissileWeaponStatusEvent E)
		{
			if (E.Override == null)
			{
				if (E.Status != null)
				{
					...
					E.Status.ammoRemaining = (IsReady(UseCharge: false, IgnoreCharge: false, IgnoreLiquid: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L, null) ? 1 : 0);
				}
				...
				if (part2 != null)
				{
					...
					else if (!part2.Cell.Understood())
					{
						E.Items.Append(" [?]");
					}
					else
					{
						part2.Cell.ForeachPartDescendedFrom(delegate(IEnergyCell part)
						{
							string text = part.ChargeStatus();
							if (text != null)
							{
		...
		public override bool HandleEvent(GetDisplayNameEvent E)
		{
			if (!ProjectileObject.IsNullOrEmpty() && E.Understood())
			{
				...
				stringBuilder.Append("{{W|").Append('\u0003').Append("}}")
					.Append(GetDamageRoll(null, null));
		...
		public string GetStatusMessage(ActivePartStatus Status)
		{
			...
			{
				return ParentObject.Does("click", int.MaxValue, null, null, "merely", AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, WithIndefiniteArticle: false, null, IndicateHidden: false, Pronoun: false, SecondPerson: true, null) + ".";
			}
							...
							if (gameObject.IsPlayer())
							{
								...
								while (true)
								{
									...
									inventory.ForeachObject(delegate(GameObject GO)
									{
										...
										{
											GO.ForeachPartDescendedFrom(delegate(IEnergyCell part)
											{
												if (SlotType == null || part.SlotType == SlotType)
												{
