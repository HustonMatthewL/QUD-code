...
namespace XRL.World.Parts
{
	...
	public class Description : IPart
	{
		...
		{
			return DifficultyEvaluation.GetDifficultyDescription(ParentObject, who, null);
		}
		...
		public override bool HandleEvent(InventoryActionEvent E)
		{
			if (E.Command == "Look")
			{
				...
				{
					stringBuilder2.Append("\n\n").Append(stringBuilder);
					if (Popup.ShowBlockPrompt(stringBuilder2.ToString(), (list.Count == 2) ? "[press {{W|space}} or recall {{W|s}}tory]" : "[press {{W|space}}]", "Sounds/UI/ui_notification", ParentObject.RenderForUI(), Capitalize: false, MuteBackground: true, CenterIcon: false, RightIcon: true, LogMessage: false) == Keys.S && list.Count == 2)
					{
						BookUI.ShowBookByID(ParentObject.Property["Story"]);
					}
				}
