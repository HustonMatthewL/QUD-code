		...
		public override bool FireEvent(Event E)
		{
			if (E.ID == "BeforeTookDamage")
			{
				...
				while (DamageTaken >= DamagePer)
				{
					...
					int i = 0;
					for (int num = Stat.Roll(Number); i < num; i++)
					{
						...
						{
							IComponent<GameObject>.XDidY(ParentObject, "burn", "off " + gameObject.an(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: false, Short: true, BaseOnly: false, IndicateHidden: false, SecondPerson: true, Reflexive: false, null));
						}
					}
				}
			}
