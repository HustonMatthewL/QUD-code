...
namespace XRL.World.Parts
{
	...
	public class ModTimereaver : IModification
	{
		...
		{
			int @for = GetSpecialEffectChanceEvent.GetFor(ParentObject.Equipped ?? ParentObject.Implantee, ParentObject, "Modification ModTimereaver TimeStop", Chance);
			E.Postfix.AppendRules("Timereaver: This weapon has " + Grammar.A(@for) + "% chance to stop time for two turns when " + ParentObject.it + ParentObject.GetVerb("hit", PrependSpace: true, PronounAntecedent: true) + ".");
		...
		public override bool FireEvent(Event E)
		{
			if (E.ID == "WeaponHit")
			{
				...
				if (GetSpecialEffectChanceEvent.GetFor(gameObjectParameter, parentObject, "Modification ModTimereaver TimeStop", Chance, subject).in100() && IsReady(UseCharge: true, IgnoreCharge: false, IgnoreLiquid: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L, null) && gameObjectParameter.FireEvent(Event.New("InitiateRealityDistortionLocal", "Object", gameObjectParameter, "Device", ParentObject, "RealityStabilizationPenetration", RealityStabilizationPenetration), E))
				{
