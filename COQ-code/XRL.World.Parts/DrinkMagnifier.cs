...
namespace XRL.World.Parts
{
	...
	public class DrinkMagnifier : IActivePart
	{
		...
		{
			if (E.ID == "ModifyDrinkEffects" && IsReady(UseCharge: true, IgnoreCharge: false, IgnoreLiquid: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L, null) && Chance.in100())
			{
		...
		public static int Magnify(GameObject Target, int Magnitude = 100)
		{
			...
			{
				Event @event = Event.New("ModifyDrinkEffects", "Magnitude", Magnitude);
				Target.FireEvent(@event);
				Magnitude = @event.GetIntParameter("Magnitude");
			}
			return Magnitude;
		}
	}
