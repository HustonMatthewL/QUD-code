	...
	public class FixitSpray : IPart
	{
		...
		public override bool HandleEvent(InventoryActionEvent E)
		{
			if (E.Command == "Apply")
			{
				...
				if (E.Item.HasTagOrProperty("NoRepair"))
				{
				...
				if (matterPhase >= 3 || !gameObject.PhaseMatches(E.Actor))
				{
					if (E.Actor.IsPlayer())
					{
						Popup.Show("Some sticky goop passes through " + gameObject.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + ".");
					}
				}
				else if (matterPhase == 2)
				{
					if (E.Actor.IsPlayer())
					{
						Popup.Show("Some sticky goop mixes in with " + gameObject.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + ".");
					}
					gameObject.LiquidVolume?.MixWith(new LiquidVolume("gel", 1), null, null, null);
				}
				else
				{
					E.Actor.PlayWorldOrUISound("Sounds/Misc/sfx_interact_artifact_repair", null);
					if (E.Actor.IsPlayer())
					{
						...
						{
							Popup.Show(gameObject.Does("are", int.MaxValue, null, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, WithIndefiniteArticle: false, null, IndicateHidden: false, Pronoun: false, SecondPerson: true, null) + " covered in sticky goop!");
						}
