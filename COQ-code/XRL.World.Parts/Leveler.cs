...
namespace XRL.World.Parts
{
	...
	public class Leveler : IPart
	{
		...
		public bool LevelUp(GameObject Kill = null, GameObject InfluencedBy = null, string ZoneID = null, IEvent ParentEvent = null)
		{
			...
			{
				beforeLevelGainedEvent.Message.Append(flag ? "You" : ParentObject.T(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: true, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null)).Append(ParentObject.Has).Append(" gained a level!")
		...
		public static void RapidAdvancement(int Amount, GameObject ParentObject)
		{
			...
			bool flag2 = false;
			if (flag && Popup.ShowYesNo("Your genome enters an excited state! Would you like to spend {{rules|4}} mutation points to buy " + Grammar.A(@for) + " before rapidly mutating?", "Sounds/UI/ui_notification_question", AllowEscape: false) == DialogResult.Yes)
			{
			...
			if (list.Count > 0)
			{
				...
				if (ParentObject.IsPlayer())
				{
					...
					int index = Popup.PickOption("Choose a physical " + @for + " to rapidly advance.", null, "", "Sounds/Misc/sfx_characterMod_mutation_windowPopup", options);
					Popup.Show("You have rapidly advanced " + list[index].DisplayName + " by " + Grammar.Cardinal(Amount) + " ranks to rank {{C|" + (list[index].Level + Amount) + "}}!", null, "Sounds/Misc/sfx_characterMod_mutation_rankUp_quickSuccession");
