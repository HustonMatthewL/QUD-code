	...
	public class ConfuseOnSight : IPart
	{
		...
		public override void TurnTick(long TimeTick, int Amount)
		{
			...
			while (enumerator.MoveNext())
			{
				...
				if (current.IsCombatObject() && !current.HasEffect(typeof(Confused)) && !current.IsMemberOfFaction(primaryFaction) && current.HasLOSTo(ParentObject) && Chance.in100() && current.FireEvent("ApplyAttackConfusion") && !current.MakeSave("Willpower", Strength, null, null, NameForChecks, IgnoreNaturals: false, IgnoreNatural1: false, IgnoreNatural20: false, IgnoreGodmode: false, ParentObject))
				{
