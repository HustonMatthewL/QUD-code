		...
		public override bool HandleEvent(RealityStabilizeEvent E)
		{
			if (TurnInto == "*fugue" && E.Check(CanDestroy: true))
			{
				...
				if (Visible())
				{
					...
					{
						IComponent<GameObject>.AddPlayerMessage(ParentObject.Poss("worldline through spacetime") + " snaps back to its canonical path, and " + ParentObject.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + ParentObject.GetVerb("vanish") + ".");
					}
					else
					{
