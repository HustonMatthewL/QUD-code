			...
			if (Duration > 0)
			{
				...
				foreach (GameObject item in ParentObject.CurrentCell.GetObjectsWithPartReadonly("Physics"))
				{
					if (item != ParentObject)
					{
						...
						item.TemperatureChange((-20 - 60 * Level) / 2, Owner, Radiant: false, MinAmbient: false, MaxAmbient: false, IgnoreResistance: false, 0, null, null);
					}
				}
				ParentObject.TemperatureChange((-20 - 60 * Level) / 2, Owner, Radiant: false, MinAmbient: false, MaxAmbient: false, IgnoreResistance: false, 0, null, null);
