		...
		public override bool FireEvent(Event E)
		{
			if (E.ID == "BeforeContentsTaken")
			{
				if (ParentObject.CurrentCell.ParentZone.GetObjectsWithTagOrProperty("CherubimLock").Count > 0)
				{
					...
					{
						EmitMessage("The protective force of the cherubim prevents " + gameObjectParameter.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + " from taking anything from the reliquary.", ' ', FromDialog: false, gameObjectParameter.IsPlayer());
					}
					return false;
				}
			}
