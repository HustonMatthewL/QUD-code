		...
		public void CheckSpawn(int Turns = 1)
		{
			...
			if (ParentObject.Physics.Temperature >= 20 && currentZone.GetObjectCount(TROLL_FOAL_BLUEPRINT) < TROLL_FOAL_LIMIT)
			{
				if (!Budding)
				{
					...
					if (IComponent<GameObject>.Visible(ParentObject))
					{
						...
						{
							IComponent<GameObject>.AddPlayerMessage("A grotesque protuberance swells from " + ParentObject.poss("back") + " as " + ParentObject.does("begin", int.MaxValue, null, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, WithIndefiniteArticle: false, null, IndicateHidden: false, Pronoun: true, SecondPerson: true, null) + " to bud!");
						}
						else
						{
							IComponent<GameObject>.AddPlayerMessage("A grotesque protuberance swells from " + ParentObject.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + " as " + ParentObject.does("begin", int.MaxValue, null, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, WithIndefiniteArticle: false, null, IndicateHidden: false, Pronoun: true, SecondPerson: true, null) + " to bud!");
						}
					}
				}
				if (Counter <= 0)
			...
			if (Budding)
			{
				...
				if (IComponent<GameObject>.Visible(ParentObject))
				{
					...
					{
						IComponent<GameObject>.AddPlayerMessage("The protuberance on " + ParentObject.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + " shrinks.");
					}
				}
			}
