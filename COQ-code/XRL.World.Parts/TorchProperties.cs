					...
					if (ParentObject.Physics.CurrentCell != null)
					{
						...
						foreach (GameObject item in ParentObject.CurrentCell.GetObjectsWithPartReadonly("Physics"))
						{
							...
							if (ParentObject.Physics.IsReal)
							{
								...
								{
									item.TemperatureChange(400, LastThrower, Radiant: false, MinAmbient: false, MaxAmbient: false, IgnoreResistance: false, phase, null, null);
								}
							}
							else if (item.Physics.Temperature < 400 && Stat.Random(1, 4) == 1)
							{
								item.TemperatureChange(100, LastThrower, Radiant: false, MinAmbient: false, MaxAmbient: false, IgnoreResistance: false, phase, null, null);
							}
						}
					}
