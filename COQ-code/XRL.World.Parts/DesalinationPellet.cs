	...
	public class DesalinationPellet : IPart
	{
		...
		public override bool HandleEvent(InventoryActionEvent E)
		{
			if (E.Command == "Apply")
			{
				...
				IComponent<GameObject>.EmitMessage(E.Actor, "You drop " + ParentObject.an(int.MaxValue, null, null, AsIfKnown: false, Single: true, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: false, Short: true, BaseOnly: false, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + " into " + gameObject.t(int.MaxValue, null, null, AsIfKnown: false, Single: true, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + ".\n\n" + PurifyLiquid(gameObject, E.Actor, ShowMessage: false, Single: true, E.Actor.IsPlayer()), ' ', FromDialog: true);
			}
			...
			if (!RemoveLiquid.IsNullOrEmpty() && LiquidVolume.GetLiquid(RemoveLiquid) != null && LV.ContainsLiquid(RemoveLiquid))
			{
				...
				if (num > 0)
				{
					...
					else
					{
						...
						LV.MixWith(new LiquidVolume(RemoveLiquid, -num3, num3), null, null, null);
					}
				}
			}
				...
				foreach (BaseLiquid allLiquid in LiquidVolume.getAllLiquids())
				{
					...
					try
					{
						...
						else
						{
							...
							LV.MixWith(new LiquidVolume(allLiquid.ID, -num4, num4), null, null, null);
						}
						if (num4 > 0)
						{
							LV.MixWith(new LiquidVolume(liquid, num4, num4), null, null, null);
							flag2 = true;
						}
					}
					finally
