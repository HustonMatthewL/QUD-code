	...
	public class QuantumRippler : IPoweredPart
	{
		...
		public override bool HandleEvent(RealityStabilizeEvent E)
		{
			if (ExplodeOnRealityStabilize && E.Check(CanDestroy: true))
			{
				...
				if (Visible())
				{
					IComponent<GameObject>.AddPlayerMessage(ParentObject.Does("collapse", int.MaxValue, null, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, WithIndefiniteArticle: false, null, IndicateHidden: false, Pronoun: false, SecondPerson: true, null) + " under the pressure of normality and" + ParentObject.GetVerb("implode") + ".");
				}
		...
		public void CheckDirection(string Direction, bool Initial = false)
		{
			...
			if (firstObjectWithPart == null || !IsReady(UseCharge: false, IgnoreCharge: false, IgnoreLiquid: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L, null))
			{
				return;
			}
			QuantumRippler part = firstObjectWithPart.GetPart<QuantumRippler>();
			if (part == null || !part.IsReady(UseCharge: false, IgnoreCharge: false, IgnoreLiquid: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L, null))
			{
				return;
			}
			while (true)
