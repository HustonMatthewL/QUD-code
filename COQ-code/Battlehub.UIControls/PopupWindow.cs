...
namespace Battlehub.UIControls
{
	public class PopupWindow : MonoBehaviour
	{
		...
		{
			m_openedPopupWindows = (from wnd in Object.FindObjectsOfType<PopupWindow>()
				where wnd.IsOpened && wnd.isActiveAndEnabled
				select wnd).ToArray();
			PopupWindow[] openedPopupWindows = m_openedPopupWindows;
			for (int i = 0; i < openedPopupWindows.Length; i++)
			{
				openedPopupWindows[i].gameObject.SetActive(value: false);
			}
