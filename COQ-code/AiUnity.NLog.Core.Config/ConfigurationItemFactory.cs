	...
	public class ConfigurationItemFactory
	{
		...
		private void RegisterExtendedItems()
		{
			...
			if (num >= 0)
			{
				...
				string @namespace = typeof(FileTarget).Namespace;
				targets.RegisterNamedType("AspNetTrace", @namespace + ".AspNetTraceTarget" + assemblyQualifiedName);
				targets.RegisterNamedType("MSMQ", @namespace + ".MessageQueueTarget" + assemblyQualifiedName);
				targets.RegisterNamedType("AspNetBufferingWrapper", @namespace + ".Wrappers.AspNetBufferingTargetWrapper" + assemblyQualifiedName);
				string namespace2 = typeof(MessageLayoutRenderer).Namespace;
				layoutRenderers.RegisterNamedType("appsetting", namespace2 + ".AppSettingLayoutRenderer" + assemblyQualifiedName);
				layoutRenderers.RegisterNamedType("aspnet-application", namespace2 + ".AspNetApplicationValueLayoutRenderer" + assemblyQualifiedName);
				layoutRenderers.RegisterNamedType("aspnet-request", namespace2 + ".AspNetRequestValueLayoutRenderer" + assemblyQualifiedName);
				layoutRenderers.RegisterNamedType("aspnet-sessionid", namespace2 + ".AspNetSessionIDLayoutRenderer" + assemblyQualifiedName);
				layoutRenderers.RegisterNamedType("aspnet-session", namespace2 + ".AspNetSessionValueLayoutRenderer" + assemblyQualifiedName);
				layoutRenderers.RegisterNamedType("aspnet-user-authtype", namespace2 + ".AspNetUserAuthTypeLayoutRenderer" + assemblyQualifiedName);
				layoutRenderers.RegisterNamedType("aspnet-user-identity", namespace2 + ".AspNetUserIdentityLayoutRenderer" + assemblyQualifiedName);
			}
		}
	}
}
