	...
	protected new void Awake()
	{
		...
		AddEventListener("onHoverMove", delegate(exUIEvent _event)
		{
			...
			else
			{
				...
				exUIPointEvent exUIPointEvent3 = _event as exUIPointEvent;
				for (int i = 0; i < exUIPointEvent3.pointInfos.Length; i++)
				{
					exUIPointInfo exUIPointInfo2 = exUIPointEvent3.pointInfos[i];
					if (draggable && (exUIPointEvent3.isTouch || exUIPointEvent3.GetMouseButton(0)) && exUIPointInfo2.id == draggingID)
					{
		...
		{
			float x2 = ((horizontalContentDir == ContentDirection.LeftToRight) ? 0f : (0f - (contentSize_.x - width)));
			float y2 = ((verticalContentDir == ContentDirection.TopToBottom) ? 0f : (0f - (contentSize_.y - height)));
			exUIWheelEvent exUIWheelEvent2 = _event as exUIWheelEvent;
			Vector2 vector2 = new Vector2(0f, (0f - exUIWheelEvent2.delta) * 100f);
			if (Mathf.Abs(exGeometryUtility.GetConstrainOffset(new Rect(scrollOffset_.x, scrollOffset_.y, width, height), new Rect(x2, y2, contentSize_.x, contentSize_.y)).y) > 0.001f)
			{
				vector2.y *= 0.5f;
			}
			velocity = Vector2.Lerp(velocity, velocity + vector2 / Time.deltaTime * scrollSpeed, 0.67f);
			if (Mathf.Sign(velocity.x) != Mathf.Sign(vector2.x))
			{
				velocity.x = 0f;
			}
			if (Mathf.Sign(velocity.y) != Mathf.Sign(vector2.y))
			{
				velocity.y = 0f;
			}
			_event.StopPropagation();
