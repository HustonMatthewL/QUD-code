...
namespace XRL.Liquids
{
	...
	public class LiquidWarmStatic : BaseLiquid
	{
		...
		private static bool GlitchSkills(GameObject Object)
		{
			...
			{
				BaseSkill baseSkill = partsDescendedFrom[i];
				...
				for (int num = 100; num > 0; num--)
				{
					...
					else if (Power != null)
					{
						...
						if (randomElement2.Cost <= 0)
						{
							continue;
						}
						name = randomElement2.Class;
					}
					int num2 = partsDescendedFrom.FindIndex((BaseSkill x) => x.Name == name);
					if (num2 == -1)
					{
						num2 = partsDescendedFrom.Count;
					...
					if (flag && i != num2)
					{
						stringBuilder.Compound(Object.Poss("knowledge of {{rules|"), '\n').Append(partsDescendedFrom[i].DisplayName).Append("}} distorts into knowledge of {{rules|")
							.Append(partsDescendedFrom[num2].DisplayName)
							.Append("}}.");
					}
					break;
				}
			...
			int j = 0;
			for (int count2 = partsDescendedFrom.Count; j < count2; j++)
			{
				if (list[j] == 1)
				{
					part.AddSkill(partsDescendedFrom[j]);
				}
				else if (list[j] == 2)
				{
					part.RemoveSkill(partsDescendedFrom[j]);
					Object.RemovePart(partsDescendedFrom[j]);
				}
			}
			return true;
		}
		...
		private static bool GlitchMutations(GameObject Object)
		{
			...
			{
				if (!MutationFactory.TryGetMutationEntry(list[i], out var Entry))
				{
					list.RemoveAt(i--);
				}
			...
			{
				BaseMutation baseMutation = list[j];
				MutationEntry entry = list3[j];
				bool defect = entry.IsDefect();
				bool single = entry.Cost <= 1;
				list4.Clear();
				list4.AddRange(from x in (from x in MutationFactory.GetCategories()
						where (rapids[j] <= 0) ? (x != MutationFactory.Morphotypes) : (x == entry.Category)
				...
				for (int num = 100; num > 0; num--)
				{
					...
					{
						int num2 = list.FindIndex((BaseMutation x) => x.Name == mutation.Class);
						BaseMutation baseMutation2 = ((num2 < 0) ? mutation.CreateInstance() : list[num2]);
						if (baseMutation2.CanLevel() == baseMutation.CanLevel())
						{
							list5[j] = baseMutation2;
							list6[j] = list[j].BaseLevel;
							rapids[j] = list[j].GetRapidLevelAmount();
							list2.Add(mutation);
							if (flag && list[j] != list5[j])
							{
								stringBuilder.Compound(Object.Poss("mutation {{rules|"), '\n').Append(list[j].DisplayName).Append("}} transmutes into the mutation {{rules|")
									.Append(list5[j].DisplayName)
									.Append("}}.");
							}
							break;
						}
			...
			{
				if (!list5.Contains(list[k]))
				{
					list[k].SetRapidLevelAmount(0, Sync: false);
					part.RemoveMutation(list[k], Sync: false);
				}
			}
			int l = 0;
			for (int count3 = list.Count; l < count3; l++)
			{
				if (list5[l].ParentObject != null)
				{
					list5[l].BaseLevel = list6[l];
				}
				else
				{
					part.AddMutation(list5[l], list6[l], Sync: false);
				}
				list5[l].SetRapidLevelAmount(rapids[l], Sync: false);
			}
		...
		public static bool GlitchLiquidComponents(GameObject Object, string LiquidTable = "RandomLiquid", int Consume = 0, bool Silent = false)
		{
			...
			if (!Silent)
			{
				...
				{
					liquidVolume.EmitMessage(Object.T(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + " starts to glitch.", 'W', FromDialog: false, gameObject != null);
				}
				else
				{
					liquidVolume.EmitMessage("The liquid mixture inside " + Object.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + " starts to glitch.", 'W', FromDialog: false, gameObject != null);
				}
			}
