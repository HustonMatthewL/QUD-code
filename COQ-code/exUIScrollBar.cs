		...
		if ((bool)transform)
		{
			...
			if ((bool)component)
			{
				...
				component.AddEventListener("onPressUp", delegate(exUIEvent _event)
				{
					...
					if ((exUIPointEvent3.isTouch || exUIPointEvent3.GetMouseButton(0)) && exUIPointEvent3.pointInfos[0].id == draggingID && dragging)
					{
						dragging = false;
						draggingID = -1;
					}
				...
				component.AddEventListener("onHoverMove", delegate(exUIEvent _event)
				{
					if ((bool)scrollView)
					{
						exUIPointEvent exUIPointEvent4 = _event as exUIPointEvent;
						for (int i = 0; i < exUIPointEvent4.pointInfos.Length; i++)
						{
							exUIPointInfo exUIPointInfo2 = exUIPointEvent4.pointInfos[i];
							if (dragging && (exUIPointEvent4.isTouch || exUIPointEvent4.GetMouseButton(0)) && exUIPointInfo2.id == draggingID)
							{
