...
namespace XRL.World.ZoneParts
{
	...
	public class ScriptCallToArms : IZonePart
	{
		...
		public void spawnParties(int p)
		{
			...
			for (int i = 3; i < 80; i++)
			{
				spawnCells = ParentZone.GetCell(location2D).GetEmptyAdjacentCells(1, i);
			...
			if (text == "*")
			{
				...
				for (int j = location2D.X - 3; j < location2D.X + 3; j++)
				{
					...
					{
						zoneFromDirection.GetCell(j, k)?.ClearWalls();
					}
				}
				for (int l = location2D.X - 3; l < location2D.X + 3; l++)
				{
					...
					{
						Cell cell = ParentZone.GetCell(l, n);
						if (25.in100())
						{
			...
			if (text == "W" && !flag2)
			{
				...
				{
					for (int num4 = ParentZone.Height - 4; num4 < ParentZone.Height; num4++)
					{
						zoneFromDirection2.GetCell(num3, num4)?.ClearWalls();
					}
				}
