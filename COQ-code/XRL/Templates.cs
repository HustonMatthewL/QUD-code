	...
	public static class Templates
	{
		...
		public class StatCollector
		{
			...
			public void CollectCooldownTurns(ActivatedAbilityEntry ability, string BaseTurns)
			{
				if (mode.Contains("ability") && ability != null)
				{
					int @base = BaseTurns.RollMinCached() * 10;
					int base2 = BaseTurns.RollMaxCached() * 10;
					GetCooldownEvent getCooldownEvent = GetCooldownEvent.TryCalculateFor(ability.ParentObject, ability, @base);
					if (getCooldownEvent != null)
					{
						GetCooldownEvent getCooldownEvent2 = GetCooldownEvent.TryCalculateFor(ability.ParentObject, ability, base2);
						int num = -(getCooldownEvent2.Result - getCooldownEvent2.Base);
						if (getCooldownEvent.Result == getCooldownEvent2.Result)
						{
							Set("Cooldown", (int)Math.Ceiling((float)getCooldownEvent.Result / 10f), num != 0, num);
						}
						else
						{
							Set("Cooldown", (int)Math.Ceiling((float)getCooldownEvent.Result / 10f) + "-" + (int)Math.Ceiling((float)getCooldownEvent2.Result / 10f), num != 0, num);
						}
						if (num == 0)
						{
							return;
