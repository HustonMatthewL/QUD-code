...
namespace XRL
{
	...
	public static class MutationFactory
	{
		...
		{
			return TryGetMutationEntry(Mutation.DisplayName, Mutation.Name, out Entry);
		}
		...
		{
			CheckInit();
			bool flag = Options.GetOption("OptionEnablePrereleaseContent", "No") == "Yes";
			foreach (MutationEntry entry in category.Entries)
			{
				if ((!entry.Prerelease || flag) && (!entry.ExcludeFromPool || IgnoreExclude))
				{
					list.Add(entry);
				}
			}
		...
		private static void Init()
		{
			...
			for (int i = 0; i < _Categories.Count; i++)
			{
				...
				for (int j = 0; j < mutationCategory.Entries.Count; j++)
				{
					...
					_MutationsByName[mutationEntry.DisplayName] = mutationEntry;
		...
		public static void HandleCategoryNode(XmlDataHelper xml)
		{
			...
			{
				text = text.TrimStart('-');
				MetricsManager.LogPotentialModError(xml.modInfo, xml.BaseURI + ":" + xml.LineNumber + ": Entry removal discontinued, set Hidden attribute instead.");
			}
		...
		public static void HandleMutationNode(XmlDataHelper xml)
		{
			...
			currentMutation = _currentParsingCategory.Entries.Find((MutationEntry x) => x.DisplayName == name);
			...
			if (num)
			{
				if (currentMutation.Prerelease && Options.GetOption("OptionEnablePrereleaseContent") != "Yes")
				{
