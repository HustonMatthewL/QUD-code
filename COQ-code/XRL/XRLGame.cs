...
namespace XRL
{
	...
	public class XRLGame : IEventSource, IEventHandler
	{
		...
		[NonSerialized]
		public const int SaveVersion = 396;

		[NonSerialized]
		public const int MinSaveVersion = 395;

		[NonSerialized]
		public const int MaxSaveVersion = 396;
		...
		public static XRLGame LoadGame(string Path, bool Session = false, bool ShowPopup = false, Dictionary<string, object> GameState = null)
		{
			...
			try
			{
				...
				try
				{
					...
					{
						text = "2.0.167.0 or prior";
						throw new Exception("Save file is the incorrect version.");
					}
					num = Reader.FileVersion;
					text = Reader.ReadString();
					if (Reader.FileVersion < 395 || Reader.FileVersion > 396)
					{
						throw new Exception("Save file is the incorrect version (" + text + ").");
					}
					...
					catch (Exception x)
					{
						...
						MetricsManager.LogException("Exception loading zone manager", x);
					}
					...
					if (Reader.FileVersion < 396)
					{
						...
						{
							MetricsManager.LogException("TauChimeCompat", x2);
						}
					}
				}
				finally
			...
			{
				int j = 0;
				for (int count = Return.Systems.Count; j < count; j++)
				{
					Return.Systems[j].ApplyRegistrar(Return);
				}
			...
			{
				MetricsManager.LogException("AfterGameLoaded", x3);
			}
			return Return;
		}
		...
		{
			XRL.World.GameObject player = The.Player;
			RenderEvent renderEvent = player.RenderForUI();
		...
		public Task SaveGame(string GameName, string Message = "Saving game", bool CopyCache = false, bool CopyPrimary = false)
		{
			...
			try
			{
				...
				Reader.Start(396, SerializePlayer: true);
				Reader.Write(123457);
