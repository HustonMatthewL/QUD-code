	...
	public class Multiweapon_Flurry : BaseSkill
	{
		...
		public bool PerformFlurry()
		{
			...
			if (combatTarget == null)
			{
				...
				if (combatTarget != null)
				{
					ParentObject.Fail("You cannot reach " + combatTarget.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + " to attack " + combatTarget.them + ".");
				}
				else
				{
			...
			@event.SetParameter("TargetObject", combatTarget);
			@event.SetParameter("TargetCell", combatTarget?.CurrentCell);
