	...
	public class Tinkering_Tinker1 : BaseSkill
	{
		...
		public bool Recharge(GameObject Object, IEvent FromEvent = null)
		{
			...
			Object.ForeachPartDescendedFrom(delegate(IRechargeable rcg)
			{
				...
				string @string = rechargeBit.GetString();
				...
				if (bitCount == 0)
				{
					return ParentObject.Fail("You don't have any " + BitType.GetString(@string) + " bits, which are required for recharging.");
				}
				int num2 = Math.Min(bitCount, num);
				int valueOrDefault = Popup.AskNumber("It would take {{C|" + num + "}} " + BitType.GetString(@string) + " " + ((num == 1) ? "bit" : "bits") + " to fully recharge " + Object.t(int.MaxValue, null, null, AsIfKnown: false, Single: true, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + ". You have {{C|" + bitCount + "}}. How many do you want to use?", "Sounds/UI/ui_notification", "", num2, 0, num2).GetValueOrDefault();
				if (valueOrDefault > 0)
				{
					...
					Popup.Show("You have " + ((valueOrDefault < num) ? "partially " : "") + "recharged " + Object.t(int.MaxValue, null, null, AsIfKnown: false, Single: true, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + ".");
					Object.CheckStack();
					AnyRecharged = true;
				}
				return true;
			...
			{
				Popup.ShowFail(Object.T(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + " can't be recharged that way.");
			}
			if (AnyRecharged)
			{
				ParentObject.UseEnergy(1000, "Skill Tinkering Recharge", null, null);
		...
		public override bool FireEvent(Event E)
		{
			if (E.ID == "CommandRechargeObject")
			{
				...
				{
					GameObject gameObject = Popup.PickGameObject("Select an item to charge.", scopeDisposedList2, AllowEscape: true, ShowContext: true);
					if (gameObject != null)
					{
