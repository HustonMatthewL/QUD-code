		...
		public override bool FireEvent(Event E)
		{
			...
			else if (E.ID == "DefenderAfterAttackMissed")
			{
				...
				if (gameObject != null)
				{
					...
					{
						IComponent<GameObject>.AddPlayerMessage("You rejoinder with " + ParentObject.poss(gameObject, Definite: true, null) + ".", 'G');
					}
					else if (gameObjectParameter.IsPlayer())
					{
						IComponent<GameObject>.AddPlayerMessage(ParentObject.The + ParentObject.ShortDisplayName + ParentObject.GetVerb("rejoinder") + " with " + ParentObject.poss(gameObject, Definite: true, null) + ".", 'R');
					}
