...
namespace Qud.UI
{
	...
	public class JournalStatusScreen : BaseStatusScreen<JournalStatusScreen>
	{
		...
		public void UpdateStatues()
		{
			...
			{
				int num = int.Parse(item.GetProperty("period", "0"));
				string property = item.GetProperty("name", "<unknown>");
				string path = item.GetProperty("statueTile", null) ?? SultanShrine.GetStatueForSultan(property, num);
				sultanStatuePeriod.Add(num);
				if (sultanStatues.Count <= i)
				{
				...
				sultanStatues[i].gameObject.SetActive(value: true);
				sultanStatues[i].SetHighlight(highlightState: false);
				sultanStatues[i].SetBase(item.GetProperty("statueBase") == "true");
				sultanStatues[i].text.SetText(property);
				sultanStatues[i].fadedImage.sprite = SpriteManager.GetUnitySprite(path);
				sultanStatues[i].fullImage.sprite = SpriteManager.GetUnitySprite(path);
				i++;
			}
			for (; i < sultanStatues.Count; i++)
			{
				sultanStatues[i].gameObject.SetActive(value: false);
			}
		}
		...
		public void updateCategoryButtonBar()
		{
			if (categoryBarButtons == null)
			{
				...
				for (int i = 0; i < categoryBarButtons.Count; i++)
				{
					int n = i;
					categoryBarButtons[i].onSelect = delegate
					{
			...
			int j = 0;
			for (int count = categoryBarButtons.Count; j < count; j++)
			{
				FilterBarCategoryButtonData filterBarCategoryButtonData = categoryBarButtons[j];
