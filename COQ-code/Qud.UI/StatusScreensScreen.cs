...
namespace Qud.UI
{
	...
	public class StatusScreensScreen : SingletonWindowBase<StatusScreensScreen>, ControlManager.IControllerChangedEvent
	{
		...
		{
			freeTabs();
			tabs.AddRange(from s in Screens
				where s != null
				select PooledFrameworkDataElement<ButtonBar.ButtonBarButtonData>.next().set(s.GetComponent<IStatusScreen>().GetTabString().ToUpper(), s.GetComponent<IStatusScreen>().GetTabIcon(), (CurrentScreen % Screens.Count == s.GetSiblingIndex()) ? ButtonBar.ButtonBarButtonData.HighlightState.Highlighted : ButtonBar.ButtonBarButtonData.HighlightState.NotHighlighted));
			for (int i = 0; i < tabs.Count; i++)
			{
				int s2 = i;
				tabs[i].onSelect = delegate
				{
					CurrentScreen = s2;
					UpdateActiveScreen();
				};
			...
			{
				ControlId.Assign(tabs[j]?.button.gameObject, $"StatusScreensScreenTab:{j}");
			}
		}
