	...
	public class PlayerStatusBar : SingletonWindowBase<PlayerStatusBar>
	{
		...
		public void Update()
		{
			if (playerCharDirty)
			{
				lock (playerChar)
				{
					PlayerBody3c.FromRenderable(The.Player?.RenderForUI());
			...
			if (timeOfDayImageDirty)
			{
				lock (timeLock)
				{
					if (!string.IsNullOrEmpty(customClock))
					{
						if (!customClocks.ContainsKey(customClock))
						{
							customClocks.Add(customClock, SpriteManager.GetUnitySprite(customClock));
						}
						TimeClockImage.sprite = customClocks[customClock];
					}
