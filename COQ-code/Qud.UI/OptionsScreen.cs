...
namespace Qud.UI
{
	...
	public class OptionsScreen : SingletonWindowBase<OptionsScreen>
	{
		...
		private OptionsMenuButtonRow searcher = new OptionsMenuButtonRow();
		...
		public async Task<bool> OptionsMenu()
		{
			...
			menuItems = Options.OptionsByCategory.SelectMany(delegate(KeyValuePair<string, List<GameOption>> kv)
			{
				...
				return kv.Value.Select(delegate(GameOption option)
				{
					...
					{
						optionsDataRow = ((!(option.Type == "Combo") && !(option.Type == "BigCombo")) ? ((OptionsDataRow<string>)new OptionsUnknownTypeRow
						{
							Id = option.ID,
							IsEnabled = () => option.Requires?.RequirementsMet ?? true,
							CategoryId = option.Category,
							Title = option.DisplayText,
							SearchWords = option.SearchKeywords + " " + option.DisplayText + " " + option.Category,
							Value = Options.GetOption(option.ID),
							Type = option.Type
						}) : ((OptionsDataRow<string>)new OptionsComboBoxRow
						{
							Id = option.ID,
							IsEnabled = () => option.Requires?.RequirementsMet ?? true,
							CategoryId = option.Category,
					...
					else
					{
						int result;
						int result2;
						int value = (int.TryParse(Options.GetOption(option.ID), out result) ? result : (int.TryParse(option.Default, out result2) ? result2 : option.Min));
						optionsDataRow = new OptionsSliderRow
						{
							Id = option.ID,
							IsEnabled = () => option.Requires?.RequirementsMet ?? true,
							CategoryId = option.Category,
					...
					optionsDataRow.HelpText = option.HelpText;
					return optionsDataRow;
				}).Prepend(element);
			}).ToList();
		...
		public void FilterItems()
		{
			...
			if (string.IsNullOrWhiteSpace(searchText))
			{
				...
				{
					foreach (IGrouping<string, OptionsDataRow> item in from i in menuItems
						where i != null && !(i is OptionsCategoryRow) && i.IsEnabled != null && i.IsEnabled()
						select i into s
