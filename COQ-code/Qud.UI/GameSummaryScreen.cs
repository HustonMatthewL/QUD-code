			...
			if (EndGame.IsAnyEnding)
			{
				...
				else
				{
					...
					if (EndGame.IsBrightsheol)
					{
						...
						cause = ((!EndGame.IsUltimate) ? "You freed the Spindle for another's ascent and crossed into Brightsheol." : "You annuled the plagues of the Gyre.\n\nThen you freed the Spindle for another's ascent and crossed into Brightsheol");
					}
					else if (EndGame.IsMarooned)
					{
						The.Game.SetStringGameState("FinalEndmark", "marooned");
						SetEndmark("marooned", EndGame.IsUltimate);
						cause = ((!EndGame.IsUltimate) ? "You destroyed Resheph to save the burgeoning world but marooned yourself at the North Sheva." : "You annuled the plagues of the Gyre.\n\nThen you destroyed Resheph to save the burgeoning world but marooned yourself at the North Sheva.");
					}
					else if (EndGame.IsCovenant)
					{
						The.Game.SetStringGameState("FinalEndmark", "covenant");
						SetEndmark("covenant", EndGame.IsUltimate);
						cause = ((!EndGame.IsUltimate) ? "You entered into a covenant with Resheph to help prepare Qud for the Coven's return." : "You annuled the plagues of the Gyre.\n\nThen you entered into a covenant with Resheph to help prepare Qud for the Coven's return.");
					}
					else if (EndGame.IsReturn)
					{
						The.Game.SetStringGameState("FinalEndmark", "return");
						SetEndmark("return", EndGame.IsUltimate);
						cause = (EndGame.IsArkOpened ? ((!EndGame.IsUltimate) ? "You destroyed Resheph and returned to Qud to help garden the burgeoning world." : "You annuled the plagues of the Gyre.\n\nThen you destroyed Resheph and returned to Qud to help garden the burgeoning world.") : ((!EndGame.IsUltimate) ? "You rebuked Resheph and returned to Qud to help garden the burgeoning world." : "You annuled the plagues of the Gyre.\n\nThen you returned to Qud to help garden the burgeoning world."));
					}
					else if (EndGame.IsAccede)
					{
						The.Game.SetStringGameState("FinalEndmark", "accede");
						SetEndmark("accede", EndGame.IsUltimate);
						cause = ((!EndGame.IsUltimate) ? "You acceded to Resheph's plan to purge the world of higher life in preparation for the Coven's return." : "You annuled the plagues of the Gyre.\n\nThen you reversed course and acceded to Resheph's plan to purge the world of higher life, in preparation for the Coven's return.");
					}
					else if (EndGame.IsLaunch)
					{
						The.Game.SetStringGameState("FinalEndmark", "spaceship");
						SetEndmark("spaceship", EndGame.IsUltimate);
						cause = (EndGame.IsArkOpened ? (EndGame.IsWithBarathrum ? ((!EndGame.IsUltimate) ? "You destroyed Resheph and launched yourself into the dusted cosmos to ply the stars with Barathrum." : "You annuled the plagues of the Gyre.\n\nThen you destroyed Resheph and launched yourself into the dusted cosmos to ply the stars with Barathrum.") : ((!EndGame.IsUltimate) ? "You destroyed Resheph and launched yourself into the dusted cosmos to ply the stars." : "You annuled the plagues of the Gyre.\n\nThen you destroyed Resheph and launched yourself into the dusted cosmos to ply the stars.")) : (EndGame.IsWithBarathrum ? ((!EndGame.IsUltimate) ? "You launched yourself into the dusted cosmos to ply the stars with Barathrum." : "You annuled the plagues of the Gyre.\n\nThen you launched yourself into the dusted cosmos to ply the stars with Barathrum.") : ((!EndGame.IsUltimate) ? "You launched yourself into the dusted cosmos to ply the stars." : "You annuled the plagues of the Gyre.\n\nThen you launched yourself into the dusted cosmos to ply the stars.")));
					}
					else
					{
