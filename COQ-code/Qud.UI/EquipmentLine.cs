...
namespace Qud.UI
{
	public class EquipmentLine : BaseLineWithTooltip, IFrameworkControl, IFrameworkControlSubcontexts, IBeginDragHandler, IEventSystemHandler, IDragHandler, IEndDragHandler, IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler
	{
		...
		public void setData(FrameworkDataElement data)
		{
			...
			if (equipmentLineData.showCybernetics)
			{
				icon.FromRenderable(equipmentLineData.bodyPart?.Cybernetics?.RenderForUI() ?? null);
			}
			else
			{
				...
				if (gameObject2 != null)
				{
					...
					{
						icon.FromRenderable(gameObject2.RenderForUI()?.GreyOutForUI());
					...
					else
					{
						...
						icon.FromRenderable((!flag) ? gameObject2?.RenderForUI() : gameObject2?.RenderForUI()?.GreyOutForUI());
					}
				}
				else
				{
					icon.FromRenderable(gameObject2?.RenderForUI());
				}
			}
			if (itemText != null)
			{
