...
namespace Qud.UI
{
	...
	public class MainMenu : SingletonWindowBase<MainMenu>, ControlManager.IControllerChangedEvent
	{
		...
		public override void Show()
		{
			...
			versionText.SetText(ModManager.MarketingVersion.ToString() + "\n{{K|build " + ModManager.CoreVersion.ToString() + "}}");
			base.Show();
		...
		public async void SelectedInfo(FrameworkDataElement data)
		{
			...
			if (mainMenuOptionData.Command == "Pick:Redeem Code")
			{
				try
				{
					string text = await Popup.AskStringAsync("Redeem a Code", "", 32, 0, null, ReturnNullForEscape: false, EscapeNonMarkupFormatting: true, null);
