...
namespace Qud.UI
{
	public class CharacterMutationLine : MonoBehaviour, IFrameworkControl, IFrameworkControlSubcontexts, IPointerClickHandler, IEventSystemHandler
	{
		...
		public void setData(FrameworkDataElement data)
		{
			...
			{
				text.SetText("{{y|" + characterMutationLineData.mutation.DisplayName + "}}");
				return;
			}
			string arg = "C";
			...
			text.SetText($"{{{{y|{characterMutationLineData.mutation.DisplayName} ({{{{{arg}|{characterMutationLineData.mutation.GetUIDisplayLevel()}}}}})}}}}");
		}
