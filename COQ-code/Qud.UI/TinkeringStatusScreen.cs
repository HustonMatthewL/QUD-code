		...
		public override void UpdateViewFromData()
		{
			...
			else if (CurrentCategory == 1)
			{
					...
					if (text != null && obj.Understood())
					{
						foreach (TinkeringLineData data2 in dataList)
						{
							if (data2.data.CanMod(text) && ItemModding.ModificationApplicable(data2.data.PartName, obj, The.Player))
							{
								...
								{
									if (!filterBarCategories.Contains(item))
									{
										filterBarCategories.Add(item);
									}
								}
				...
				foreach (string category in usedCategories)
				{
					...
					TinkeringLineData item2 = PooledFrameworkDataElement<TinkeringLineData>.next().set(this, CurrentCategory, category: true, category, list.Count, !isCollapsed(category), 0, tinkeringLineData.data, null);
					...
					if (list.Count() == 0)
					{
						TinkeringLineData item3 = PooledFrameworkDataElement<TinkeringLineData>.next().set(this, CurrentCategory, category: false, category, 0, categoryExpanded: false, 1, tinkeringLineData.data, null);
					...
					foreach (GameObject item9 in list)
					{
						...
						TinkeringLineData item4 = tinkeringLineData2.set(this, currentCategory, category: false, category, 0, categoryExpanded: false, num4 + 1, tinkeringLineData.data, modObject);
						dataToFree.Add(item4);
						listItems.Add(item4);
						num3++;
						num4++;
					}
