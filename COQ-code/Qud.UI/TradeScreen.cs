...
namespace Qud.UI
{
	...
	public class TradeScreen : SingletonWindowBase<TradeScreen>, ControlManager.IControllerChangedEvent
	{
		...
		public static string TradeScreenVerb
		{
			get
			{
				if (TradeUI.ScreenMode != 0 || !(TradeUI.costMultiple > 0f))
				{
					return "transfer";
				}
				return "trade";
		...
		public void UpdateViewFromData()
		{
			...
			{
				obj.ForEach(delegate(TradeLineData i)
				{
					i.free();
				});
				obj.Clear();
			...
			{
				foreach (TradeEntry item in array2[j])
				{
					if (item.GO != null)
					{
			...
			for (int k = 0; k < 2; k++)
			{
				FrameworkScroller frameworkScroller = scrollerControllers[k];
				List<TradeEntry> list2 = tradeEntries[k];
				List<string> list3 = usedCategories[k];
				Dictionary<string, List<TradeEntry>> dictionary = objectCategories[k];
				List<TradeLineData> list4 = listItems[k];
				...
				{
					filteredEntries.AddRange(list2.Where((TradeEntry i) => filterBar.enabledCategories.Contains("*All") || filterBar.enabledCategories.Contains(i.CategoryName) || (i.GO != null && filterBar.enabledCategories.Contains(i.GO.GetInventoryCategory()))));
				}
				else
				{
					searcher.CategoryName = searchText;
					filteredEntries.AddRange(from m in Process.ExtractTop(searcher, list2, (TradeEntry i) => i.NameForSort.ToLower(), null, list2.Count, 50)
						select m.Value into i
						where filterBar.enabledCategories.Contains("*All") || filterBar.enabledCategories.Contains(i.CategoryName) || (i.GO != null && filterBar.enabledCategories.Contains(i.GO.GetInventoryCategory()))
						select i);
				}
				...
				if (sortMode == SortMode.Category)
				{
					...
					{
						list4.Add(PooledFrameworkDataElement<TradeLineData>.next().set(k, TradeLineDataType.Category, TradeLineDataStyle.Interact, null, null, traderInventory: false, 0, item3, isCollapsed(k, item3)));
						if (isCollapsed(k, item3))
						{
							continue;
						}
						...
						foreach (TradeEntry item4 in dictionary[item3])
						{
							...
							int j = howManySelected(item4.GO);
							list4.Add(tradeLineData.set(side, TradeLineDataType.Item, TradeLineDataStyle.Interact, gO, item4, k == 0, j, item3, collapsed: false, indent: true));
						}
					}
				}
				else if (sortMode == SortMode.AZ)
				{
					...
					foreach (TradeEntry item5 in filteredEntries.Where((TradeEntry i) => i.GO != null))
					{
						TradeLineData tradeLineData2 = PooledFrameworkDataElement<TradeLineData>.next();
						XRL.World.GameObject gO2 = item5.GO;
						int j = howManySelected(item5.GO);
						list4.Add(tradeLineData2.set(0, TradeLineDataType.Item, TradeLineDataStyle.Interact, gO2, item5, k == 0, j, null, collapsed: false, indent: true));
					}
				}
		...
		public void UpdateTitleBars()
		{
			if (Trader != null)
			{
				traderIcons[0].FromRenderable(Trader.RenderForUI());
			}
			traderIcons[1].FromRenderable(The.Player.RenderForUI());
		...
		public async void HandleMenuOption(FrameworkDataElement element)
		{
			...
			{
				int num = selectedSide;
				if (num > -1)
				{
					bool flag = false;
					for (int i = 0; i < listItems[num].Count; i++)
					{
						if (listItems[num][i].go != null && howManySelected(listItems[num][i].go) < listItems[num][i].go.Count)
						{
							flag = true;
							break;
						}
					...
					if (flag)
					{
						foreach (TradeLineData item in listItems[num])
						{
							if (item.go != null)
							{
					...
					{
						foreach (TradeLineData item2 in listItems[num])
						{
							if (item2.go != null)
							{
		...
		public void ReselectAfterCategoryStateChange(TradeLineData item)
		{
			for (int j = 0; j < 2; j++)
			{
				...
				if (item.go != null)
				{
					num = listItems[j].FindIndex((TradeLineData i) => i.go == item.go);
				}
				if (num == -1)
				{
					num = listItems[j].FindIndex((TradeLineData i) => i.category == item.category);
				}
				num = Math.Max(0, Math.Min(num, listItems[j].Count - 1));
				scrollerControllers[j].selectedPosition = num;
				scrollerControllers[j].ScrollSelectedIntoView();
				scrollerControllers[j].scrollContext.GetContextAt(num).Activate();
			}
		}
