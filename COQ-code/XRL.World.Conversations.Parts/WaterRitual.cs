	...
	public class WaterRitual : IConversationPart
	{
		...
		public override bool HandleEvent(DisplayTextEvent E)
		{
			...
			E.Text.Append(" is {{C|").Append(RecordFaction.CurrentReputation).Append("}}.\n")
				.Append(RecordFaction.Visible ? The.Speaker.T(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: true, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) : The.Speaker.It)
		...
		public void ModifyReputation()
		{
			...
			int @for = GetWaterRitualReputationAmountEvent.GetFor(The.Player, The.Speaker, Record, Record.faction, num);
			@for = @for * performance / 100;
			if (@for != 0)
			{
				The.Game.PlayerReputation.Modify(RecordFaction, @for, null, null, "WaterRitualPrimaryAward");
			}
			if (part == null)
			{
				return;
			...
			foreach (KeyValuePair<string, int> item in The.Speaker.Brain.Allegiance)
			{
				if (!(item.Key == Record.faction))
				{
					...
					if (ifExists != null)
					{
						int amount = GetWaterRitualReputationAmountEvent.GetFor(The.Player, The.Speaker, Record, Record.faction, num) * performance / 100;
