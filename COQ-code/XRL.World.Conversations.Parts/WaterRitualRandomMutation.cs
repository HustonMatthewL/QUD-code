	...
	public class WaterRitualRandomMutation : IWaterRitualPart
	{
		...
		public override void Awake()
		{
			...
			if (WaterRitual.Record.TryGetAttribute(text, out var Value))
			{
				...
				{
					WaterRitual.Record.attributes.Remove(text + Entry.DisplayName);
					Entry = null;
				}
			}
			if (Entry == null)
		...
		public override bool HandleEvent(EnteredElementEvent E)
		{
			...
			else if (UseReputation())
			{
				...
				{
					Popup.Show(Message.StartReplace().AddObject(The.Speaker).AddReplacer("mutation.name", "{{M|" + Entry.DisplayName + "}}")
						.StripColors()
						.ToString());
				}
		...
		{
			E.Tag = "{{" + Lowlight + "|[gain {{M|" + Entry.DisplayName + "}}: {{" + Numeric + "|" + GetReputationCost() + "}} reputation]}}";
			return false;
		}
	}
}
