		...
		public bool Execute()
		{
			...
			for (int count = scopeDisposedList.Count; i < count && num > 0; i++)
			{
				...
				if (Remove)
				{
					...
					{
						Popup.ShowFail("You cannot give " + gameObject3.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + "!");
						continue;
					}
				}
				else
