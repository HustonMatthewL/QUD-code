	...
	public class EndGame : IConversationPart
	{
		...
		public static async Task RunCreditsAsync()
		{
			...
			GameManager.Instance.uiQueue.queueTask(delegate
			{
				...
				if (IsBrightsheol)
				{
					...
					text2 = ((!IsUltimate) ? "You freed the Spindle for another's ascent and crossed into Brightsheol." : "You annuled the plagues of the Gyre.\n\nThen you freed the Spindle for another's ascent and crossed into Brightsheol");
				}
				else if (IsMarooned)
				{
					The.Game.SetStringGameState("FinalEndmark", "marooned");
					component.SetEndmark("marooned", IsUltimate);
					text2 = ((!IsUltimate) ? "You destroyed Resheph to save the burgeoning world but marooned yourself at the North Sheva." : "You annuled the plagues of the Gyre.\n\nThen you destroyed Resheph to save the burgeoning world but marooned yourself at the North Sheva.");
				}
				else if (IsCovenant)
				{
					The.Game.SetStringGameState("FinalEndmark", "covenant");
					component.SetEndmark("covenant", IsUltimate);
					text2 = ((!IsUltimate) ? "You entered into a covenant with Resheph to help prepare Qud for the Coven's return." : "You annuled the plagues of the Gyre.\n\nThen you entered into a covenant with Resheph to help prepare Qud for the Coven's return.");
				}
				else if (IsReturn)
				{
					The.Game.SetStringGameState("FinalEndmark", "return");
					component.SetEndmark("return", IsUltimate);
					text2 = (IsArkOpened ? ((!IsUltimate) ? "You destroyed Resheph and returned to Qud to help garden the burgeoning world." : "You annuled the plagues of the Gyre.\n\nThen you destroyed Resheph and returned to Qud to help garden the burgeoning world.") : ((!IsUltimate) ? "You rebuked Resheph and returned to Qud to help garden the burgeoning world." : "You annuled the plagues of the Gyre.\n\nThen you returned to Qud to help garden the burgeoning world."));
				}
				else if (IsAccede)
				{
					The.Game.SetStringGameState("FinalEndmark", "accede");
					component.SetEndmark("accede", IsUltimate);
					text2 = ((!IsUltimate) ? "You acceded to Resheph's plan to purge the world of higher life in preparation for the Coven's return." : "You annuled the plagues of the Gyre.\n\nThen you reversed course and acceded to Resheph's plan to purge the world of higher life, in preparation for the Coven's return.");
				}
				else if (IsLaunch)
				{
					The.Game.SetStringGameState("FinalEndmark", "spaceship");
					component.SetEndmark("spaceship", IsUltimate);
					text2 = (IsArkOpened ? (IsWithBarathrum ? ((!IsUltimate) ? "You destroyed Resheph and launched yourself into the dusted cosmos to ply the stars with Barathrum." : "You annuled the plagues of the Gyre.\n\nThen you destroyed Resheph and launched yourself into the dusted cosmos to ply the stars with Barathrum.") : ((!IsUltimate) ? "You destroyed Resheph and launched yourself into the dusted cosmos to ply the stars." : "You annuled the plagues of the Gyre.\n\nThen you destroyed Resheph and launched  yourself into the dusted cosmos to ply the stars.")) : (IsWithBarathrum ? ((!IsUltimate) ? "You launched yourself into the dusted cosmos to ply the stars with Barathrum." : "You annuled the plagues of the Gyre.\n\nThen you launched yourself into the dusted cosmos to ply the stars with Barathrum.") : ((!IsUltimate) ? "You launched yourself into the dusted cosmos to ply the stars." : "You annuled the plagues of the Gyre.\n\nThen you launched yourself into the dusted cosmos to ply the stars.")));
				}
				else
				{
			...
			GameManager.Instance.uiQueue.queueTask(delegate
			{
				...
				UnityEngine.GameObject gameObject2 = UIManager.mainCanvas.transform.Find("EndgameCredits").gameObject;
		...
		public static void GoCoda()
		{
			...
			try
			{
				...
				gameObject.SystemMoveTo(The.PlayerCell, null);
					...
					.TryGetPart<Interior>(out var Part))
				{
					...
					gameObject.SystemMoveTo(pullDownLocation, null);
					gameObject.GiveProperName(null, Force: false, null, SpecialFaildown: false, null, null);
					ClearHistory();
			...
			finally
			{
				...
				float? to = 0f;
				FadeToBlack.Fade(3f, null, to, null, null);
		...
		private static void EvaluateVillageFactions()
		{
			...
			for (int num = list.Count / 2; i < num; i++)
			{
				...
				faction.DisplayName = "villagers of " + NameStyles.NameStyleTable["Chiliad Qudish Site"].Generate(null, null, null, null, null, null, null, null, null, null, null, "Site", null, FailureOkay: false, SpecialFaildown: false, null, null, null, null, null, null);
