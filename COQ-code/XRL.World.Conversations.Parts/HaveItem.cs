	...
	public class HaveItem : IConversationPart
	{
		...
		public bool PlayerHasAmount()
		{
			...
			if (Require && array.Length != 0)
			{
				...
				The.Player.ShowFailure((Amount > 1) ? ("You do not have enough " + Grammar.Pluralize(gameObject2.DisplayNameOnlyDirectAndStripped) + ".") : ("You do not have " + gameObject2.an(int.MaxValue, null, null, AsIfKnown: false, Single: true, NoConfusion: false, NoColor: false, Stripped: true, WithoutTitles: false, Short: true, BaseOnly: false, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + "."));
			}
			return false;
		}
	}
