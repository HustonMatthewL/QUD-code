	...
	public static class QudHistoryFactory
	{
		...
		public static void GenerateNewSultan(History history, int period)
		{
			...
			for (int j = 0; j < entitiesByDelegate.Count; j++)
			{
				...
				bool flag = false;
				for (int k = 0; k < historicEntity.events.Count; k++)
				{
					if (historicEntity.events[k].HasEventProperty("revealsRegion") && historicEntity.events[k].GetEventProperty("revealsRegion") == property)
					{
						flag = true;
						break;
					}
		...
		public static string NameRuinsSite(History history, out bool Proper, out string nameRoot)
		{
			...
			nameRoot = NameMaker.MakeName(null, null, null, null, "Qudish", null, null, null, null, null, null, "Site", null, FailureOkay: false, SpecialFaildown: false, null, null);
		...
		public static HistoricEntity RequirePlayerEntity()
		{
			...
			HistoricEvent historicEvent2 = (historicEvent = obj.events[0]);
			Dictionary<string, string> dictionary = historicEvent.entityProperties ?? (historicEvent.entityProperties = new Dictionary<string, string>());
			...
			Dictionary<string, List<string>> dictionary2 = historicEvent.addedListProperties ?? (historicEvent.addedListProperties = new Dictionary<string, List<string>>());
