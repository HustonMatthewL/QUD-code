...
public static class CombatJuice
{
	...
	public static void _playPrefabAnimation(Vector3 location, string name, string objectId = null, string configurationString = null)
	{
		UnityEngine.GameObject animation = getInstance(name, "PrefabAnimation_" + name);
		animation.transform.position = location;
		animation.GetComponent<ICombatJuiceConfigurable>()?.configure(configurationString);
	...
	public static CombatJuiceEntryPrefabAnimation playPrefabAnimation(Vector3 location, string animation, string objectID = null, string configurationString = null, bool async = false)
	{
		CombatJuiceEntryPrefabAnimation obj = new CombatJuiceEntryPrefabAnimation(location, animation, objectID, configurationString)
		{
			async = async
		};
	...
	public static CombatJuiceEntryPrefabAnimation playPrefabAnimation(XRL.World.GameObject gameObject, string animation, string objectId = null, string configurationString = null, bool async = false)
	{
		if (gameObject != null && gameObject.IsValid() && gameObject.IsVisible())
		{
			return playPrefabAnimation(gameObject.Physics.CurrentCell.Location, animation, objectId, configurationString, async);
		}
		return null;
	}

	public static CombatJuiceEntryPrefabAnimation playPrefabAnimation(Location2D cellLocation, string animation, string objectId = null, string configurationString = null, bool async = false)
	{
		return playPrefabAnimation(gameManager.getTileCenter(cellLocation.X, cellLocation.Y, 100), animation, objectId, configurationString, async);
	}
}
