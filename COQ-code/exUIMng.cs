...
public class exUIMng : MonoBehaviour
{
	...
	public static exUIMng inst
	{
		get
		{
			if (inst_ == null)
			{
				inst_ = Object.FindObjectOfType(typeof(exUIMng)) as exUIMng;
			}
			return inst_;
		}
	}
	...
	private void Init()
	{
		...
		exUIControl[] array = Object.FindObjectsOfType(typeof(exUIControl)) as exUIControl[];
	...
	public void HoverOut(exUIControl _ctrl, int _id)
	{
		...
		foreach (exHotPoint exHotPoint2 in array)
		{
			...
			exUIPointInfo exUIPointInfo2 = default(exUIPointInfo);
			exUIPointInfo2.id = exHotPoint2.id;
			exUIPointInfo2.pos = exHotPoint2.pos;
			exUIPointInfo2.delta = exHotPoint2.delta;
			exUIPointInfo2.worldPos = exHotPoint2.worldPos;
			exUIPointInfo2.worldDelta = exHotPoint2.worldDelta;
	...
	private void HandleHotPoints(exHotPoint[] _hotPoints, bool _isMouse)
	{
		...
		foreach (exHotPoint exHotPoint3 in _hotPoints)
		{
			if (exHotPoint3.active && exHotPoint3.pressDown)
			{
				...
				{
					exUIPointInfo exUIPointInfo3 = default(exUIPointInfo);
					exUIPointInfo3.id = exHotPoint3.id;
					exUIPointInfo3.pos = exHotPoint3.pos;
					exUIPointInfo3.delta = exHotPoint3.delta;
					exUIPointInfo3.worldPos = exHotPoint3.worldPos;
					exUIPointInfo3.worldDelta = exHotPoint3.worldDelta;
		...
		foreach (KeyValuePair<exUIControl, List<exHotPoint>> item in dictionary)
		{
			...
			for (int m = 0; m < item.Value.Count; m++)
			{
				...
				exUIPointInfo exUIPointInfo4 = default(exUIPointInfo);
				exUIPointInfo4.id = exHotPoint5.id;
				exUIPointInfo4.pos = exHotPoint5.pos;
				exUIPointInfo4.delta = exHotPoint5.delta;
				exUIPointInfo4.worldPos = exHotPoint5.worldPos;
				exUIPointInfo4.worldDelta = exHotPoint5.worldDelta;
		...
		foreach (exHotPoint exHotPoint6 in _hotPoints)
		{
			...
			exUIPointInfo exUIPointInfo5 = default(exUIPointInfo);
			exUIPointInfo5.id = exHotPoint6.id;
			exUIPointInfo5.pos = exHotPoint6.pos;
			exUIPointInfo5.delta = exHotPoint6.delta;
			exUIPointInfo5.worldPos = exHotPoint6.worldPos;
			exUIPointInfo5.worldDelta = exHotPoint6.worldDelta;
