...
namespace XRL.World.Capabilities
{
	public static class Firefighting
	{
		...
		public static bool AttemptFirefighting(GameObject Actor, GameObject Subject, int EnergyCost = 1000, bool Automatic = false, bool Dialog = false)
		{
			...
			if (num && EnergyCost > 0)
			{
				Actor.UseEnergy(1000, Automatic ? "Pass Firefighting" : "Firefighting", null, null);
			}
			return num;
		}
		...
		private static bool AttemptFirefightingCore(GameObject Actor, GameObject Subject, int EnergyCost, bool Automatic, bool Dialog)
		{
			...
			if (Actor == Subject && Actor.HasEffect<Prone>() && !Actor.IsFlying && Actor.PhaseMatches(1))
			{
				...
				int @for = GetFirefightingPerformanceEvent.GetFor(Actor, null, Patting: false, Rolling: true);
				if (@for != 0)
				{
					Actor.TemperatureChange(@for, null, Radiant: false, MinAmbient: true, MaxAmbient: false, IgnoreResistance: false, 5, null, null);
				}
				return true;
			}
			if (bodyPart != null)
			{
				if (Actor != Subject)
				{
					...
					if (Actor.IsPlayer())
					{
						int num = GlobalState.instance.intState.get("FirefightingCount", 0);
						num++;
						GlobalState.instance.intState.set("FirefightingCount", num);
					}
				}
				if (Actor == Subject)
				{
				...
				int for2 = GetFirefightingPerformanceEvent.GetFor(Actor, Subject, Patting: true);
				if (for2 != 0)
				{
					Subject.TemperatureChange(for2, null, Radiant: false, MinAmbient: true, MaxAmbient: false, IgnoreResistance: false, 5, null, null);
				}
				return true;
			}
			if (Actor != Subject)
			...
			if (flag)
			{
				...
				Messaging.XDidY(Actor, "roll", "on the ground", "!", null, null, Actor, null, UseFullNames: false, IndefiniteSubject: false, null, null, DescribeSubjectDirection: false, DescribeSubjectDirectionLate: false, AlwaysVisible: false, Dialog);
				Subject.TemperatureChange(-200, null, Radiant: false, MinAmbient: true, MaxAmbient: false, IgnoreResistance: false, 5, null, null);
				return true;
			}
			return false;
		}
