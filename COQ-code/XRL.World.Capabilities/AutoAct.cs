		...
		public static bool TryToMove(GameObject Actor, Cell FromCell, ref GameObject LastDoor, Cell ToCell = null, string Direction = null, bool AllowDigging = true, bool OpenDoors = true, bool Peaceful = true, bool PostMoveHostileCheck = true, bool PostMoveSidebarCheck = true)
		{
			...
			if (OpenDoors)
			{
				...
				for (int count = ToCell.Objects.Count; i < count; i++)
				{
					...
					{
						Interrupt("you cannot keep " + gameObject2.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null) + " open", null, gameObject2);
						return false;
					}
					...
					@event.SetFlag("UsePopupsForFailures", Actor.IsPlayer());
					@event.SetFlag("FromMove", State: true);
					if (!Part.FireEvent(@event))
					{
						if (!gameObject2.IsCreature)
						{
							Interrupt("you cannot open " + gameObject2.t(int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, null, IndicateHidden: false, SecondPerson: true, Reflexive: false, null), null, gameObject2);
							return false;
						}
						continue;
					}
			...
			else
			{
				...
				if (!Actor.Move(direction, out var Blocking, Forced: false, System: false, IgnoreGravity: false, NoStack: false, AllowDashing: true, DoConfirmations: true, null, null, NearestAvailable: false, null, null, null, peaceful))
				{
					if (Blocking != null)
					{
						Interrupt(Blocking.does("are", int.MaxValue, null, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: true, Short: true, BaseOnly: false, WithIndefiniteArticle: false, null, IndicateHidden: false, Pronoun: false, SecondPerson: true, null) + " in the way", null, Blocking);
					}
					else
					{
