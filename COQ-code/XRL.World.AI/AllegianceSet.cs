		...
		public int GetBaseFeeling(Faction Faction, IDictionary<string, int> Override = null)
		{
			...
			if (num > 0f)
			{
				for (int i = 0; i < Length; i++)
				{
					if (Slots[i].Key != null)
					{
						num2 = ((Override == null || !Override.TryGetValue(Slots[i].Key, out var value)) ? (num2 + (float)Faction.GetFeelingTowardsFaction(Slots[i].Key, null) * ((float)Slots[i].Value / num)) : (num2 + (float)value * ((float)Slots[i].Value / num)));
					}
				}
			}
			return Mathf.RoundToInt(num2);
