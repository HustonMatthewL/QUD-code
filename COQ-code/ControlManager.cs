...
public class ControlManager : MonoBehaviour
{
	...
	public static LegacyKeyCode mapCommandToPrimaryLegacyKeycode(string id)
	{
		...
		foreach (KeyValuePair<string, Dictionary<string, int>> item in CommandBindingManager.CurrentMap.PrimaryMapCommandToKeyLayer)
		{
			...
			{
				LegacyKeyCode result = default(LegacyKeyCode);
				result.rawCode = value;
				result.code = value2;
				result.bCtrl = (value & 0x20000) > 0;
				result.bShift = (value & 0x10000) > 0;
				result.bAlt = (value & 0x40000) > 0;
				return result;
			}
		}
		LegacyKeyCode result2 = default(LegacyKeyCode);
		result2.code = UnityEngine.KeyCode.None;
		return result2;
	}
