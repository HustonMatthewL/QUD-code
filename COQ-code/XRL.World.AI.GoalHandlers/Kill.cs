				...
				if (missileWeapons != null)
				{
					...
					for (int count = missileWeapons.Count; i < count; i++)
					{
						...
						if (gameObject.GetPart<EnergyCellSocket>() != null)
						{
							...
							if (part != null && part.GetActivePartStatus(UseCharge: false, IgnoreCharge: false, IgnoreLiquid: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L, null) == ActivePartStatus.Unpowered)
							{
								flag = CommandReloadEvent.Execute(base.ParentObject, gameObject, null, FreeAction: false, FromDialog: false, part.GetActiveChargeUse(null, null));
								break;
							}
							int @for = QueryDrawEvent.GetFor(gameObject);
							if (@for > 0 && !gameObject.TestCharge(@for, LiveOnly: false, 0L) && !gameObject.HasEffect<ElectromagneticPulsed>())
							{
								flag = CommandReloadEvent.Execute(base.ParentObject, gameObject, null, FreeAction: false, FromDialog: false, @for);
								break;
							}
						}
					}
		...
		public bool TryMissileWeapon()
		{
			...
			if (base.ParentObject.GetIntProperty("TurretWarmup") > 0 && (target.IsPlayerControlled() || Visible()))
			{
				if (base.ParentObject.GetIntProperty("TurretWarmup") == 1)
				{
					...
					base.ParentObject.UseEnergy(1000, "Warmup", null, null);
